...
class ReleaseRuntimeClosureTest(unittest.TestCase):
    ...
    def test_run_text_command_uses_python36_compatible_subprocess_kwargs(self):
        sentinel = object()

        with mock.patch.object(
            release_runtime_closure.subprocess,
            "run",
            return_value = sentinel,
        ) as run_mock:
            result = release_runtime_closure.run_text_command(["ldd", "/tmp/libxls.so"])

        self.assertIs(result, sentinel)
        run_mock.assert_called_once_with(
            ["ldd", "/tmp/libxls.so"],
            check = False,
            stdout = release_runtime_closure.subprocess.PIPE,
            stderr = release_runtime_closure.subprocess.PIPE,
    ...
    def test_validate_matching_linux_release_target_rejects_unsupported_host_with_override_hint(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            tmp_path = Path(tmp_dir)
            os_release_path = tmp_path / "os-release"
            os_release_path.write_text(
            ...
            with self.assertRaisesRegex(
                ...
                release_runtime_closure.validate_matching_linux_release_target(
                    release_runtime_closure.ReleaseTarget.UBUNTU2004,
                    os_release_path = os_release_path,
                )
    ...
    def test_package_runtime_closure_allows_unsupported_host_with_override(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            tmp_path = Path(tmp_dir)
            os_release_path = tmp_path / "os-release"
            os_release_path.write_text(
            ...
            with mock.patch.object(
                ...
                result = release_runtime_closure.package_runtime_closure(
                    primary_dso_path = libxls_path,
                    output_dir = tmp_path,
                    release_target = release_runtime_closure.ReleaseTarget.UBUNTU2004,
                    allow_release_target_mismatch = True,
                    os_release_path = os_release_path,
                )
