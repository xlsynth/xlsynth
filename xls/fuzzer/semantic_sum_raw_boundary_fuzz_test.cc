...
TEST(SemanticSumRawBoundaryFuzzTest, ReplaysSeedsAndRandomRoundtrips) {
  XLS_ASSERT_OK_AND_ASSIGN(auto temp_dir, TempDirectory::Create());
  XLS_ASSERT_OK_AND_ASSIGN(
      auto stats,
      RunSemanticSumRawBoundaryFuzz({
  ...
  EXPECT_EQ(stats.manifest_seed_cases_verified, 3);
  EXPECT_GT(stats.valid_roundtrips_verified, 0);
  EXPECT_GT(stats.invalid_rejections_verified, 0);
