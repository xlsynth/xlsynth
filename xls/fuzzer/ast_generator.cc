...
absl::Status AstGeneratorOptions::Validate() const {
  if (require_cross_module_sum_type && generate_proc) {
...
AstGenerator::RequiredSumType& AstGenerator::GetOrCreateRequiredSumType() {
  ...
  const bool use_sparse_discriminants =
      options_.max_width_bits_types >= 3 && RandomBool(0.5);
  const bool use_signed_discriminants =
      use_sparse_discriminants && options_.emit_signed_types && RandomBool(0.5);
  TypeAnnotation* tag_type =
      use_sparse_discriminants
          ? MakeTypeAnnotation(use_signed_discriminants, /*width=*/3,
                               /*use_xn=*/false)
          : nullptr;
  std::optional<Expr*> unit_discriminant;
  std::optional<Expr*> active_discriminant;
  if (use_sparse_discriminants) {
    unit_discriminant = GenerateNumber(/*value=*/0, tag_type);
    active_discriminant = GenerateNumber(
        use_signed_discriminants ? /*value=*/-1 : /*value=*/3, tag_type);
  }
  ...
  if (options_.max_width_bits_types > 0) {
    ...
    active_variant = module_->Make<SumVariant>(
        ...
        std::vector<StructMemberNode*>{}, active_discriminant);
  ...
  auto* sum_def = module_->Make<SumDef>(
      ...
      std::move(variants), /*is_public=*/false, tag_type);
...
TypeRefTypeAnnotation* AstGenerator::MakeRequiredSumTypeAnnotation() {
  ...
  const int64_t tag_bits =
      sum_type.sum_def->tag_type_annotation() == nullptr
          ? 1
          : GetTypeBitCount(sum_type.sum_def->tag_type_annotation());
  type_bit_counts_[type_ref_type->ToString()] = tag_bits + payload_bits;
...
AstGenerator::GenerateRequiredProcSumInput(
  ...
  statements->push_back(module_->Make<Statement>(
      module_->Make<Let>(fake_span_, recv_tuple_def, recv_tuple_type, recv_expr,
                         /*is_const=*/false)));
  ...
  auto make_tuple_member = [&](int64_t index, TypeAnnotation* member_type,
    ...
    statements->push_back(module_->Make<Statement>(module_->Make<Let>(
        fake_span_, member_def, member_type,
...
absl::Status AstGenerator::GenerateRequiredProcSumOutput(
  ...
  statements->push_back(module_->Make<Statement>(
      module_->Make<Let>(fake_span_, send_token_def, token.type, send_expr,
                         /*is_const=*/false)));
