...
TEST(SemanticSumInhabitanceFuzzTest, ReplaysSeededAndRandomValues) {
  XLS_ASSERT_OK_AND_ASSIGN(auto temp_dir, TempDirectory::Create());
  XLS_ASSERT_OK_AND_ASSIGN(
      auto stats,
      RunSemanticSumInhabitanceFuzz({
  ...
  EXPECT_GT(stats.seeded_values_verified, 0);
  EXPECT_EQ(stats.generated_values_verified, 32);
