...
struct xls_bvalue* xls_builder_base_add_select(struct xls_builder_base* builder,
  ...
  xls::BValue bvalue = cpp_builder->Select(cpp_selector, cpp_cases, cpp_default, xls::SourceInfo(), name ? name : "");
  auto* cpp_heap_bvalue = new xls::BValue(bvalue);
  return reinterpret_cast<xls_bvalue*>(cpp_heap_bvalue);
}
...
struct xls_bvalue* xls_builder_base_add_clz(struct xls_builder_base* builder,
  ...
  xls::BValue bvalue = cpp_builder->Clz(cpp_value, xls::SourceInfo(), name ? name : "");
  auto* cpp_heap_bvalue = new xls::BValue(bvalue);
  return reinterpret_cast<xls_bvalue*>(cpp_heap_bvalue);
}
...
struct xls_bvalue* xls_builder_base_add_ctz(struct xls_builder_base* builder,
  ...
  auto* cpp_builder = reinterpret_cast<xls::BuilderBase*>(builder);
  xls::BValue cpp_value = *reinterpret_cast<xls::BValue*>(value);
  xls::BValue bvalue = cpp_builder->Ctz(cpp_value, xls::SourceInfo(), name ? name : "");
  auto* cpp_heap_bvalue = new xls::BValue(bvalue);
  return reinterpret_cast<xls_bvalue*>(cpp_heap_bvalue);
}

struct xls_bvalue* xls_builder_base_add_after_all(struct xls_builder_base* builder,
                                                  struct xls_bvalue** dependencies,
  ...
  xls::BValue bvalue = cpp_builder->AfterAll(cpp_deps, xls::SourceInfo(), name ? name : "");
  auto* cpp_heap_bvalue = new xls::BValue(bvalue);
  return reinterpret_cast<xls_bvalue*>(cpp_heap_bvalue);
}

struct xls_bvalue* xls_builder_base_add_array_slice(struct xls_builder_base* builder,
                                                    struct xls_bvalue* array,
                                                    struct xls_bvalue* start,
                                                    int64_t width,
                                                    const char* name) {
  auto* cpp_builder = reinterpret_cast<xls::BuilderBase*>(builder);
  xls::BValue cpp_array = *reinterpret_cast<xls::BValue*>(array);
  xls::BValue cpp_start = *reinterpret_cast<xls::BValue*>(start);
  xls::BValue bvalue = cpp_builder->ArraySlice(cpp_array, cpp_start, width, xls::SourceInfo(), name ? name : "");
  auto* cpp_heap_bvalue = new xls::BValue(bvalue);
  return reinterpret_cast<xls_bvalue*>(cpp_heap_bvalue);
}

struct xls_bvalue* xls_builder_base_add_array_update(struct xls_builder_base* builder,
                                                     struct xls_bvalue* array,
                                                     struct xls_bvalue* update_value,
                                                     struct xls_bvalue** indices,
                                                     int64_t index_count,
  ...
  xls::BValue bvalue = cpp_builder->ArrayUpdate(cpp_array, cpp_update, cpp_indices, assumed_in_bounds, xls::SourceInfo(), name ? name : "");
  auto* cpp_heap_bvalue = new xls::BValue(bvalue);
  return reinterpret_cast<xls_bvalue*>(cpp_heap_bvalue);
}

struct xls_bvalue* xls_builder_base_add_array_concat(struct xls_builder_base* builder,
                                                     struct xls_bvalue** arrays,
                                                     int64_t array_count,
                                                     const char* name) {
  auto* cpp_builder = reinterpret_cast<xls::BuilderBase*>(builder);
  ...
  xls::BValue bvalue = cpp_builder->ArrayConcat(cpp_arrays, xls::SourceInfo(), name ? name : "");
  auto* cpp_heap_bvalue = new xls::BValue(bvalue);
  return reinterpret_cast<xls_bvalue*>(cpp_heap_bvalue);
}

struct xls_bvalue* xls_builder_base_add_identity(struct xls_builder_base* builder,
                                                 struct xls_bvalue* value,
                                                 const char* name) {
  auto* cpp_builder = reinterpret_cast<xls::BuilderBase*>(builder);
  xls::BValue cpp_value = *reinterpret_cast<xls::BValue*>(value);
  xls::BValue bvalue = cpp_builder->Identity(cpp_value, xls::SourceInfo(), name ? name : "");
  auto* cpp_heap_bvalue = new xls::BValue(bvalue);
  return reinterpret_cast<xls_bvalue*>(cpp_heap_bvalue);
}
...
struct xls_bvalue* xls_builder_base_add_encode(struct xls_builder_base* builder,
  ...
  auto* cpp_builder = reinterpret_cast<xls::BuilderBase*>(builder);
  xls::BValue cpp_value = *reinterpret_cast<xls::BValue*>(value);
  xls::BValue bvalue = cpp_builder->Encode(cpp_value, xls::SourceInfo(), name ? name : "");
  auto* cpp_heap_bvalue = new xls::BValue(bvalue);
  return reinterpret_cast<xls_bvalue*>(cpp_heap_bvalue);
}
...
struct xls_bvalue* xls_builder_base_add_decode(struct xls_builder_base* builder,
  ...
  xls::BValue bvalue = cpp_builder->Decode(cpp_value, cpp_width, xls::SourceInfo(), name ? name : "");
  auto* cpp_heap_bvalue = new xls::BValue(bvalue);
  return reinterpret_cast<xls_bvalue*>(cpp_heap_bvalue);
}
