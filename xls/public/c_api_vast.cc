...
struct xls_vast_if_statement* xls_vast_statement_block_add_if(
  ...
  xls::verilog::Conditional* cpp_if =
      cpp_block->Add<xls::verilog::Conditional>(xls::SourceInfo(), cpp_cond);
  return reinterpret_cast<xls_vast_if_statement*>(cpp_if);
}

struct xls_vast_statement_block* xls_vast_if_statement_get_then_block(
    struct xls_vast_if_statement* if_stmt) {
  auto* cpp_if = reinterpret_cast<xls::verilog::Conditional*>(if_stmt);
  return reinterpret_cast<xls_vast_statement_block*>(cpp_if->consequent());
}
...
struct xls_vast_statement_block* xls_vast_if_statement_add_else_if(
  ...
  auto* cpp_if = reinterpret_cast<xls::verilog::Conditional*>(if_stmt);
  auto* cpp_cond = reinterpret_cast<xls::verilog::Expression*>(cond);
  xls::verilog::StatementBlock* block = cpp_if->AddAlternate(cpp_cond);
...
struct xls_vast_statement_block* xls_vast_if_statement_add_else(
    struct xls_vast_if_statement* if_stmt) {
  auto* cpp_if = reinterpret_cast<xls::verilog::Conditional*>(if_stmt);
  xls::verilog::StatementBlock* block = cpp_if->AddAlternate(nullptr);
  return reinterpret_cast<xls_vast_statement_block*>(block);
}
