...
TEST(XlsCApiTest, VastProceduralControlAndBlocking) {
  ...
  xls_vast_if_statement* if_stmt = xls_vast_statement_block_add_if(
      block, xls_vast_logic_ref_as_expression(pred1));
  ASSERT_NE(if_stmt, nullptr);
  xls_vast_statement_block* then_block =
      xls_vast_if_statement_get_then_block(if_stmt);
  ...
  xls_vast_statement_block* elseif_block =
      xls_vast_if_statement_add_else_if(if_stmt, and_expr);
  ...
  xls_vast_statement_block* else_block =
      xls_vast_if_statement_add_else(if_stmt);
