...
// TODO(seanhaskell): Re-enable state bits checks for new FSM once it settles
#define ExpectStateBitsForProc(name, bits)                             \
  {                                                                    \
    if (!generate_new_fsm_) {                                          \
      XLS_ASSERT_OK_AND_ASSIGN(uint64_t top_proc_state_bits,           \
                               GetStateBitsForProcNameContains(name)); \
      EXPECT_EQ(top_proc_state_bits, bits);                            \
    }                                                                  \
  }
...
class TranslatorProcTest_OldFSMOnly
    : public TranslatorProcTestWithoutFSMParam,
      public ::testing::WithParamInterface<TestParams> {
 protected:
  TranslatorProcTest_OldFSMOnly() {
    generate_new_fsm_ = GetParam().generate_new_fsm;
    merge_states_ = GetParam().merge_states;
    split_states_on_channel_ops_ = GetParam().split_states_on_channel_ops;
  }
...
INSTANTIATE_TEST_SUITE_P(
    TranslatorProcTest_OldFSMOnly, TranslatorProcTest_OldFSMOnly,
    Values(TestParams{.generate_new_fsm = false,
                      .merge_states = false,
                      .split_states_on_channel_ops = false},
           TestParams{.generate_new_fsm = false,
...
TEST_P(TranslatorProcTest_OldFSMOnly, IOProcMuxInSubroutine) {
  const std::string content = R"(
    void sub_mux(const int& dir,
              __xls_channel<int>& in,
              __xls_channel<int>& out1,
              __xls_channel<int> &out2) {
      const int ctrl = in.read();
...
TEST_P(TranslatorProcTest, ForPipelined) {
  ...
  ExpectStateBitsForProc("foo", loop_body_bits);
}

TEST_P(TranslatorProcTest, ForPipelinedDirectIn) {
  const std::string content = R"(
    #pragma hls_top
    void foo(int& swreg,
             __xls_channel<int>& out) {
  ...
  HLSBlock block_spec;
  {
    ...
    HLSChannel* ch_swreg = block_spec.add_channels();
    ch_swreg->set_name("swreg");
    ch_swreg->set_is_input(true);
    ch_swreg->set_type(CHANNEL_TYPE_DIRECT_IN);
  ...
  absl::flat_hash_map<std::string, std::list<xls::Value>> inputs;
  inputs["swreg"] = {xls::Value(xls::SBits(2, 32))};

  {
    absl::flat_hash_map<std::string, std::list<xls::Value>> outputs;
    outputs["out"] = {xls::Value(xls::SBits(2 + 10, 32))};

    ProcTest(content, block_spec, inputs, outputs);
  }
}

TEST_P(TranslatorProcTest, ForPipelinedOneBypass) {
  const std::string content = R"(
    #pragma hls_top
    void foo(__xls_channel<int>& in,
                    __xls_channel<int>& out) {
  ...
  absl::flat_hash_map<std::string, std::list<xls::Value>> inputs;
  inputs["in"] = {xls::Value(xls::SBits(3, 32)), xls::Value(xls::SBits(10, 32)),
                  xls::Value(xls::SBits(100, 32)),
                  xls::Value(xls::SBits(33, 32))};

  {
    absl::flat_hash_map<std::string, std::list<xls::Value>> outputs;
    outputs["out"] = {xls::Value(xls::SBits(2 * 3 + 3 * (10 + 100 + 33), 32))};
...
TEST_P(TranslatorProcTest, ForPipelinedFSMInside6) {
  ...
  HLSBlock block_spec;
  {
    ...
    ch_in3->set_type(CHANNEL_TYPE_FIFO);
...
TEST_P(TranslatorProcTest, ForPipelinedJustAssign) {
  ...
  ExpectStateBitsForProc("foo", loop_proc_bits);
}
...
TEST_P(TranslatorProcTest, DoWhilePipelined) {
  ...
  HLSBlock block_spec;
  {
    ...
    ac_in->set_type(CHANNEL_TYPE_FIFO);

    HLSChannel* bc_in = block_spec.add_channels();
    bc_in->set_name("bc");
    bc_in->set_is_input(true);
    bc_in->set_type(CHANNEL_TYPE_FIFO);
  ...
  ExpectStateBitsForProc("foo", 1 + 64 + 32);
}
...
TEST_P(TranslatorProcTest, ForPipelinedSerial) {
  ...
  ExpectStateBitsForProc("foo", 32 + 1 + 64 + 1 + 16 + 32);
}
...
TEST_P(TranslatorProcTest_OldFSMOnly, ForPipelinedReturnInBody) {
  const std::string content = R"(
    #include "/xls_builtin.h"
...
TEST_P(TranslatorProcTest, ForPipelinedMoreVars) {
  ...
  ExpectStateBitsForProc("foo", 16 + 1 + 32 + 64);
}
...
// This test is checking the proper passing of parameters for statics
// and a top object at the same time.
TEST_P(TranslatorProcTest, ClassPlusStatics) {
  const std::string content = R"(
    class Block {
    public:
      __xls_channel<int, __xls_channel_dir_Out>& out;
      ...
      #pragma hls_top
      void foo() {
        static int reg_st = 10;
        static int reg_st2 = 7;

        out.write(100 + reg_st2);

        ++st;

        out.write(200 * reg_st);
  ...
  {
    absl::flat_hash_map<std::string, std::list<xls::Value>> outputs;
    outputs["out"] = {xls::Value(xls::SBits(100 + 7, 32)),
                      xls::Value(xls::SBits(200 * 10, 32)),
                      xls::Value(xls::SBits(300, 32)),
                      xls::Value(xls::SBits(4 + 10, 32)),

                      xls::Value(xls::SBits(100 + 9, 32)),
                      xls::Value(xls::SBits(200 * 20, 32)),
                      xls::Value(xls::SBits(300, 32)),
                      xls::Value(xls::SBits(5 + 20, 32)),

                      xls::Value(xls::SBits(100 + 11, 32)),
                      xls::Value(xls::SBits(200 * 30, 32)),
                      xls::Value(xls::SBits(300, 32)),
                      xls::Value(xls::SBits(6 + 30, 32))};
...
TEST_P(TranslatorProcTest, ForPipelinedPreCondBreak) {
  ...
  ExpectStateBitsForProc("foo", 1 + 32 + 64);
}
...
TEST_P(TranslatorProcTest, ForPipelinedBreak) {
  ...
  HLSBlock block_spec;
  {
    ...
    ch_out_i->set_type(CHANNEL_TYPE_FIFO);
  }
...
TEST_P(TranslatorProcTest_OldFSMOnly, ForPipelinedInFunction) {
  const std::string content = R"(
    #include "/xls_builtin.h"
  ...
  ExpectStateBitsForProc("foo", loop_bits);
}

TEST_P(TranslatorProcTest_OldFSMOnly, ForPipelinedInMethod) {
  const std::string content = R"(
    #include "/xls_builtin.h"
...
TEST_P(TranslatorProcTest_OldFSMOnly, ForPipelinedInMethodWithMember) {
  const std::string content = R"(
    #include "/xls_builtin.h"
  ...
  ExpectStateBitsForProc("foo", 32 + loop_bits);
}

TEST_P(TranslatorProcTest_OldFSMOnly, ForPipelinedInFunctionInIf) {
  const std::string content = R"(
    #include "/xls_builtin.h"
...
TEST_P(TranslatorProcTest_OldFSMOnly, ForPipelinedStaticInBody) {
  const std::string content = R"(
    #pragma hls_top
    void foo(__xls_channel<int>& in,
             __xls_channel<int>& out) {
...
TEST_P(TranslatorProcTest, ForPipelinedStaticOuter2) {
  ...
  ExpectStateBitsForProc("foo", 32 + 16 + 1 + 64);
}
...
TEST_P(TranslatorProcTest, ForPipelinedNested) {
  ...
  ExpectStateBitsForProc("foo", 32 + 1 + 16 + 1 + 64);
}
...
TEST_P(TranslatorProcTest, ForPipelinedNestedBreak) {
  ...
  HLSBlock block_spec;
  {
    ...
    chA_in->set_type(CHANNEL_TYPE_FIFO);

    HLSChannel* chB_in = block_spec.add_channels();
    chB_in->set_name("inB");
    chB_in->set_is_input(true);
    chB_in->set_type(CHANNEL_TYPE_FIFO);
...
TEST_F(TranslatorProcTestWithoutFSMParam, ForPipelinedCheckFSM) {
  ...
  XLS_ASSERT_OK_AND_ASSIGN(
      state_by_io_node, GetStatesByIONodeForFSMProc(/*func_name=*/"foo_impl"));
...
TEST_F(TranslatorProcTestWithoutFSMParam, ForPipelinedMergeCheckFSM) {
  ...
  // Test full merging
  {
    ...
    absl::flat_hash_map<xls::Node*, int64_t> state_by_io_node;
    XLS_ASSERT_OK_AND_ASSIGN(state_by_io_node, GetStatesByIONodeForFSMProc(
                                                   /*func_name=*/"foo_impl"));
...
TEST_P(TranslatorProcTest, ForPipelinedNested2) {
  ...
  ExpectStateBitsForProc("foo", 32 + 1 + 64 + 1 + 64);
}
...
TEST_P(TranslatorProcTest, ForPipelinedNestedInheritIIWithLabel) {
  ...
  ExpectStateBitsForProc("foo", 32 + 3 * (1 + 64));
}
...
TEST_P(TranslatorProcTest, ForPipelinedNestedWithIO2) {
  ...
  HLSBlock block_spec;
  {
    ...
    ch_init->set_type(CHANNEL_TYPE_FIFO);
...
TEST_P(TranslatorProcTest_OldFSMOnly, ForPipelinedIOInBodySubroutine) {
  const std::string content = R"(
...
TEST_P(TranslatorProcTest_OldFSMOnly, ForPipelinedIOInBodySubroutine2) {
  const std::string content = R"(
...
TEST_P(TranslatorProcTest_OldFSMOnly, ForPipelinedIOInBodySubroutineMultiCall) {
  const std::string content = R"(
...
TEST_P(TranslatorProcTest_OldFSMOnly, ForPipelinedIOInBodySubSubroutine) {
  const std::string content = R"(
    #include "/xls_builtin.h"
...
TEST_P(TranslatorProcTest_OldFSMOnly, ForPipelinedIOInBodySubSubroutine2) {
  const std::string content = R"(
    #include "/xls_builtin.h"
  ...
  ExpectStateBitsForProc("foo", loop_bits + innermost_loop_bits);
}

TEST_P(TranslatorProcTest_OldFSMOnly, ForPipelinedIOInBodySubSubroutine3) {
  const std::string content = R"(
...
TEST_P(TranslatorProcTest_OldFSMOnly, ForPipelinedIOInBodySubSubroutine4) {
  const std::string content = R"(
    #include "/xls_builtin.h"
  ...
  ExpectStateBitsForProc("foo", sub_bits + sub_sub_bits + sub_sub_sub_bits);
}

TEST_P(TranslatorProcTest_OldFSMOnly, ForPipelinedIOInBodySubroutineDeclOrder) {
  const std::string content = R"(
    #include "/xls_builtin.h"
...
TEST_P(TranslatorProcTest_OldFSMOnly, ForPipelinedIOInBodySubroutine3) {
  const std::string content = R"(
    #include "/xls_builtin.h"
...
TEST_P(TranslatorProcTest_OldFSMOnly, ForPipelinedIOInBodySubroutine4) {
  const std::string content = R"(
    int sub_read(__xls_channel<int>& in2) {
      int a = 0;
      #pragma hls_pipeline_init_interval 1
  ...
  ExpectStateBitsForProc("foo", 32 + 1 + 64 + 1 + 32);
}
...
TEST_P(TranslatorProcTest_OldFSMOnly, PipelinedLoopUsingMemberChannel) {
  const std::string content = R"(
       #include "/xls_builtin.h"
       struct Foo {
...
TEST_P(TranslatorProcTest_OldFSMOnly,
       PipelinedLoopUsingMemberChannelAndVariable) {
  const std::string content = R"(
       struct Foo {
         __xls_channel<int>& out_;
         int accum_ = 10;
...
TEST_P(TranslatorProcTest_OldFSMOnly,
       IOProcClassWithPipelinedLoopInSubroutine) {
  const std::string content = R"(
       class Block {
        public:
         __xls_channel<int , __xls_channel_dir_In>& in;
...
TEST_P(TranslatorProcTest_OldFSMOnly, IOProcClassSubClass) {
  const std::string content = R"(
       class Sub {
        private:
         int val = 1;
...
TEST_P(TranslatorProcTest_OldFSMOnly, IOProcClassSubClass2) {
  const std::string content = R"(
       class Sub {
        private:
         int val = 1;
...
TEST_P(TranslatorProcTest, IOProcClassPropagateVars) {
  ...
  ExpectStateBitsForProc("Block_proc", 32);
}
...
TEST_P(TranslatorProcTest_OldFSMOnly, IOProcClassHierarchicalLValue) {
  const std::string content = R"(
    struct LeafBlock {
...
TEST_P(TranslatorProcTest_OldFSMOnly, DebugAssertInSubroutine) {
  const std::string content = R"(
       class Block {
        public:
         __xls_channel<int, __xls_channel_dir_In>& in;
...
TEST_P(TranslatorProcTest_OldFSMOnly, NonblockingReadInSubroutine) {
  const std::string content = R"(
       class Block {
        public:
         __xls_channel<int, __xls_channel_dir_In>& in;
...
TEST_P(TranslatorProcTest_OldFSMOnly, LocalChannelUsedInSubroutine) {
  const std::string content = R"(
    class Block {
      __xls_channel<int, __xls_channel_dir_In> in;
      __xls_channel<int, __xls_channel_dir_Out> out;
...
TEST_P(TranslatorProcTest_OldFSMOnly, LocalChannelDeclaredInSubroutine) {
  const std::string content = R"(
    class Block {
      __xls_channel<int, __xls_channel_dir_In> in;
      __xls_channel<int, __xls_channel_dir_Out> out;
...
TEST_P(TranslatorProcTest_OldFSMOnly,
       LocalChannelPipelinedLoopDeclaredInSubroutine) {
  const std::string content = R"(
    class Block {
      __xls_channel<int, __xls_channel_dir_In> in;
      __xls_channel<int, __xls_channel_dir_Out> out;
...
TEST_P(TranslatorProcTest_OldFSMOnly,
       LocalChannelPipelinedLoopUsedInSubroutine) {
  const std::string content = R"(
    class Block {
      __xls_channel<int, __xls_channel_dir_In> in;
      __xls_channel<int, __xls_channel_dir_Out> out;
...
TEST_P(TranslatorProcTest_OldFSMOnly, LocalChannelSubBlock) {
  const std::string content = R"(
    class Block {
      __xls_channel<int, __xls_channel_dir_In> in;
      __xls_channel<int, __xls_channel_dir_Out> out;
...
TEST_P(TranslatorProcTest_OldFSMOnly, LocalChannelPassUpAndDown) {
  const std::string content = R"(
    class Block {
      __xls_channel<int, __xls_channel_dir_In> in;
      __xls_channel<int, __xls_channel_dir_Out> out;
...
TEST_P(TranslatorProcTest_OldFSMOnly, ChannelDeclaredInClassInitialized) {
  const std::string content = R"(
    class Impl {
    private:
...
TEST_P(TranslatorProcTest_OldFSMOnly, LocalChannelDeclaredInClass) {
  const std::string content = R"(
    class Impl {
    private:
      __xls_channel<int, __xls_channel_dir_InOut> internal;
...
TEST_P(TranslatorProcTest_OldFSMOnly, LocalChannelDeclaredInClassNonStatic) {
  const std::string content = R"(
    class Impl {
    private:
      __xls_channel<int, __xls_channel_dir_InOut> internal;
...
TEST_P(TranslatorProcTest_OldFSMOnly, LocalChannelAddCondition) {
  const std::string content = R"(
    class Block {
      __xls_channel<int, __xls_channel_dir_In> in;
      __xls_channel<int, __xls_channel_dir_Out> out;
...
TEST_P(TranslatorProcTest_OldFSMOnly, LocalChannelAddCondition2) {
  const std::string content = R"(
    class Block {
      __xls_channel<int, __xls_channel_dir_In> in;
      __xls_channel<int, __xls_channel_dir_Out> out;
...
TEST_P(TranslatorProcTest_OldFSMOnly,
       LocalChannelDeclaredInClassNonStaticAddCondition) {
  const std::string content = R"(
    class Impl {
    private:
      __xls_channel<int, __xls_channel_dir_InOut> internal;
...
TEST_P(TranslatorProcTest_OldFSMOnly, PipelinedLoopASAPDataDependency) {
  const std::string content = R"(
       class Block {
        public:
         __xls_channel<int, __xls_channel_dir_Out>& out;
...
TEST_P(TranslatorProcTest_OldFSMOnly, PipelinedLoopSerialAfterASAP) {
  const std::string content = R"(
       class Block {
        public:
         __xls_channel<int, __xls_channel_dir_Out>& out;
...
TEST_P(TranslatorProcTest, ForPipelinedWithDirectIn) {
  ...
  ExpectStateBitsForProc("foo", 1 + 32);
}
...
TEST_P(TranslatorProcTest, ForPipelinedSerialShared) {
...

  {
    ...
    ProcTest(content, block_spec, inputs, outputs,
             /* min_ticks = */ generate_new_fsm_ ? 1 : 6 * 3);
  }

  ExpectStateBitsForProc("foo", 32 + 1 + 16 + 1 + 16 + 32);
}
...
TEST_P(TranslatorProcTest, ForPipelinedNestedShared) {
  ...
  ExpectStateBitsForProc("foo", 32 + 1 + 16 + 1 + 16);
}
...
TEST_P(TranslatorProcTest, ForPipelinedStaticShared) {
  ...
  ExpectStateBitsForProc("foo", 32 + 1 + 16 + 1 + 16 + 1 + 16);
}

TEST_P(TranslatorProcTest_OldFSMOnly, ForPipelinedStaticSharedInBody) {
  const std::string content = R"(
    #pragma hls_top
    void foo(__xls_channel<int>& in,
             __xls_channel<int>& out) {
  ...
  ExpectStateBitsForProc("foo", 32 + 1 + 16 + 32 + 1 + 16);
}
...
TEST_P(TranslatorProcTest, ReadInStoredInStatic) {
  ...
  HLSBlock block_spec;
  {
    ...
    dir_in->set_name("dir");
    dir_in->set_is_input(true);
    dir_in->set_type(CHANNEL_TYPE_FIFO);
...
TEST_P(TranslatorProcTest_OldFSMOnly, MultiBlock) {
  const std::string content = R"(
...
TEST_P(TranslatorProcTest_OldFSMOnly, MultiBlock2) {
  const std::string content = R"(
    #pragma hls_design block
    void block_a(__xls_channel<int, __xls_channel_dir_In>& in,
                 __xls_channel<int, __xls_channel_dir_InOut>& xfer) {
...
TEST_P(TranslatorProcTest_OldFSMOnly, MultiBlock3) {
  const std::string content = R"(
    #pragma hls_design block
    void block_a(__xls_channel<int, __xls_channel_dir_In>& in,
                 __xls_channel<int, __xls_channel_dir_InOut>& xfer) {
...
TEST_P(TranslatorProcTest_OldFSMOnly, MultiBlockSharedFunc) {
  const std::string content = R"(
    int shared(int x) {
      return x;
    }
...

  {
    ...
    for (const std::unique_ptr<xls::Function>& func : package_->functions()) {
      if (absl::StrContains(func->name(), "shared")) {
        ++found;
      }
    }
    EXPECT_EQ(found, 1) << " instances of shared function";
  }
}

TEST_P(TranslatorProcTest_OldFSMOnly, MultiBlockSubSub) {
  const std::string content = R"(
    #pragma hls_design block
    void block_a_sub(__xls_channel<int, __xls_channel_dir_In>& in,
                 __xls_channel<int, __xls_channel_dir_InOut>& xfer) {
      int a = in.read();
      xfer.write(a + 1);
    }
...
TEST_P(TranslatorProcTest_OldFSMOnly, MultiBlockDirectIn) {
  const std::string content = R"(
    #pragma hls_design block
    void block_a(__xls_channel<int, __xls_channel_dir_In>& in,
                 __xls_channel<int, __xls_channel_dir_InOut>& xfer) {
  ...
  HLSBlock block_spec;
  {
    ...
    swreg_in->set_type(CHANNEL_TYPE_DIRECT_IN);
...
TEST_P(TranslatorProcTest_OldFSMOnly, MultiBlockDirectInWithStatic) {
  const std::string content = R"(
    #pragma hls_design block
    void block_a(__xls_channel<int, __xls_channel_dir_In>& in,
                 __xls_channel<int, __xls_channel_dir_InOut>& xfer) {
...
TEST_P(TranslatorProcTest_OldFSMOnly, MultiBlockDirectInNonConst) {
  const std::string content = R"(
    #pragma hls_design block
    void block_a(__xls_channel<int, __xls_channel_dir_In>& in,
                 __xls_channel<int, __xls_channel_dir_InOut>& xfer) {
...
TEST_P(TranslatorProcTest_OldFSMOnly, MultiBlockAsClass) {
  const std::string content = R"(
       class Block {
        public:
         __xls_channel<int , __xls_channel_dir_In>& in;
  ...
  ASSERT_THAT(ret.status(), absl_testing::StatusIs(
                                absl::StatusCode::kUnimplemented,
                                testing::HasSubstr("methods not supported")));
}

TEST_P(TranslatorProcTest_OldFSMOnly, MultiBlockMultipleChannelCombos) {
  const std::string content = R"(
    #pragma hls_design block
    void block_a(__xls_channel<int, __xls_channel_dir_In>& in,
                 __xls_channel<int, __xls_channel_dir_InOut>& xfer) {
    ...
    #pragma hls_top
    void foo(__xls_channel<int, __xls_channel_dir_In>& in1,
             __xls_channel<int, __xls_channel_dir_In>& in2,
             __xls_channel<int, __xls_channel_dir_Out>& out) {
      static __xls_channel<int, __xls_channel_dir_InOut> xfer;
      block_a(in1, xfer);
...
TEST_P(TranslatorProcTest_OldFSMOnly, MultiBlockIRSim) {
  const std::string content = R"(
    #pragma hls_design block
...
TEST_P(TranslatorProcTest_OldFSMOnly, MultiBlockReturn) {
  const std::string content = R"(
    #pragma hls_design block
...
TEST_P(TranslatorProcTest_OldFSMOnly, MultiBlockPipelinedLoopSubBlock) {
  const std::string content = R"(
      #pragma hls_design block
      void block_a(__xls_channel<int, __xls_channel_dir_In>& in,
                  __xls_channel<int, __xls_channel_dir_InOut>& xfer) {
...
TEST_P(TranslatorProcTest, ForFullyUnrolled) {
  const std::string content = R"(
    #pragma hls_top
    void foo(__xls_channel<int>& in,
             __xls_channel<int>& out) {
  ...
  // 1 iteration on 4 inputs, single state
  int64_t min_ticks = 4, max_ticks = 4;
...
TEST_P(TranslatorProcTest, ForPartiallyUnrolled) {
  const std::string content = R"(
    #pragma hls_top
    void foo(__xls_channel<int>& in,
             __xls_channel<int>& out) {
...
TEST_P(TranslatorProcTest, ForFullyPipelinedInferredByDefault) {
  const std::string content = R"(
    #pragma hls_top
    void foo(__xls_channel<int>& in,
             __xls_channel<int>& out) {
...
TEST_P(TranslatorProcTest, ForFullyPipelinedNoDefault) {
  const std::string content = R"(
    #pragma hls_top
    void foo(__xls_channel<int>& in,
             __xls_channel<int>& out) {
  ...
  XLS_ASSERT_OK(ScanFile(content, /*clang_argv=*/{},
                         ...
                         /*error_on_init_interval=*/false));
  package_ = std::make_unique<xls::Package>("my_package");
  auto ret = translator_->GenerateIR_Block(package_.get(), block_spec);
  ASSERT_THAT(ret.status(),
              absl_testing::StatusIs(absl::StatusCode::kUnimplemented,
                                     testing::HasSubstr("missing")));
}

TEST_P(TranslatorProcTest, ForFullyPipelinedInferredInner) {
  const std::string content = R"(
    #pragma hls_top
    void foo(__xls_channel<int>& in,
             __xls_channel<int>& out) {
