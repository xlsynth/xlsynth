...
std::vector<TypeDim> SumType::GetAllDims() const {
  std::vector<TypeDim> results = {
      zero_selection_.has_value() ? storage_tag_bit_count() : tag_bit_count()};
  for (const SumTypeVariant& variant : variants()) {
...
std::unique_ptr<Type> SumType::CloneToUnique() const {
  return std::unique_ptr<Type>(
      new SumType(data_, zero_selection_, semantic_zero_selection_));
}

TypeDim SumType::storage_tag_bit_count() const { return tag_bit_count(); }
