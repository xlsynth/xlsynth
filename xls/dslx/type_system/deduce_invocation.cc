...
// Resolves a `UseTreeEntry` to a `Function` (for purposes of doing an invocation).
static absl::StatusOr<Function*> ResolveUsedFunctionForInvocation(UseTreeEntry* use_tree_entry, TypeInfo* type_info) {
  XLS_ASSIGN_OR_RETURN(ImportedInfo* imported_info, type_info->GetImportedOrError(use_tree_entry));
  ...
  XLS_ASSIGN_OR_RETURN(Function* fn, GetMemberOrTypeInferenceError<Function>(imported_info->module, name_def->identifier(), name_def->span()));
  return fn;
}
  ...
  auto resolve_fn = [](const Instantiation* node,
    ...
    if (auto* colon_ref = dynamic_cast<ColonRef*>(callee);
      ...
      if (auto* use_tree_entry = dynamic_cast<UseTreeEntry*>(definer);
        ...
        XLS_ASSIGN_OR_RETURN(fn, ResolveUsedFunctionForInvocation(use_tree_entry, ctx->type_info()));
      } else {
