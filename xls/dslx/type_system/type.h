...
class TypeVisitor {
 public:
  ...
  virtual absl::Status HandleSum(const SumType&) = 0;
...
class TypeVisitorWithDefault : public TypeVisitor {
 public:
  ...
  absl::Status HandleSum(const SumType&) override { return absl::OkStatus(); }
...
class SumType : public Type {
 public:
  ...
  const ZeroSelection& zero_selection() const {
    return zero_selection_.has_value() ? *zero_selection_
                                       : semantic_zero_selection_;
  }
 ...
 private:
  ...
  explicit SumType(std::shared_ptr<const Data> data,
                   std::optional<ZeroSelection> zero_selection,
                   ZeroSelection semantic_zero_selection)
      : data_(std::move(data)),
        zero_selection_(std::move(zero_selection)),
        semantic_zero_selection_(std::move(semantic_zero_selection)) {}

  std::shared_ptr<const Data> data_;
  std::optional<ZeroSelection> zero_selection_;
  ZeroSelection semantic_zero_selection_ = NoZeroVariant{};
};
