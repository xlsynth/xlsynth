...
TEST(TypeTest, InheritedConstructionPreservesWidthAndZeroSelection) {
  FileTable file_table;
  Module module("test", std::nullopt, file_table);
  SumDef* inner_def = CreateTupleSumDef(module, {1, 1});
  auto make_variants = [&] {
    std::vector<SumTypeVariant> variants;
    std::vector<std::unique_ptr<Type>> narrow;
    narrow.push_back(BitsType::MakeU8());
    variants.push_back(SumTypeVariant::MakeTuple(*inner_def->variants()[0],
                                                 std::move(narrow)));
    std::vector<std::unique_ptr<Type>> wide;
    wide.push_back(std::make_unique<BitsType>(false, 16));
    variants.push_back(
        SumTypeVariant::MakeTuple(*inner_def->variants()[1], std::move(wide)));
    return variants;
  };
  SumType inherited(*inner_def, make_variants(),
                    SumType::SelectedZeroVariant{*inner_def->variants()[1]});
  EXPECT_THAT(inherited.GetTotalBitCount(),
              IsOkAndHolds(TypeDim::CreateU32(25)));
  EXPECT_THAT(inherited.CloneToUnique()->GetTotalBitCount(),
              IsOkAndHolds(TypeDim::CreateU32(25)));
  EXPECT_EQ(&std::get<SumType::SelectedZeroVariant>(inherited.zero_selection())
                 .variant.get(),
            inner_def->variants()[1]);
  SumType absent(*inner_def, make_variants(), SumType::NoZeroVariant{});
  EXPECT_TRUE(
      std::holds_alternative<SumType::NoZeroVariant>(absent.zero_selection()));

  auto shared = std::make_unique<SumType>(*inner_def, make_variants());
  EXPECT_THAT(shared->GetTotalBitCount(), IsOkAndHolds(TypeDim::CreateU32(17)));
  SumDef* outer_def = CreateTupleSumDef(module, {1});
  std::vector<std::unique_ptr<Type>> members;
  members.push_back(std::move(shared));
  std::vector<SumTypeVariant> variants;
  variants.push_back(
      SumTypeVariant::MakeTuple(*outer_def->variants()[0], std::move(members)));
  SumType outer(*outer_def, std::move(variants));
  EXPECT_THAT(outer.GetMaxPayloadBitCount(),
              IsOkAndHolds(TypeDim::CreateU32(17)));
  EXPECT_THAT(outer.GetTotalBitCount(), IsOkAndHolds(TypeDim::CreateU32(17)));
}
