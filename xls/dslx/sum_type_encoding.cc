...
Phase1SumTypeEncoding::VariantInfo Phase1SumTypeEncoding::ToVariantInfo(
...
absl::Status Phase1SumTypeEncoding::ValidateVariantInfo(
  ...
  const StoredVariant& stored_variant = variants_.at(variant.variant_index);
  if (stored_variant.variant->variant().identifier() !=
      variant.variant->variant().identifier()) {
    return absl::InvalidArgumentError(absl::StrCat(
        "VariantInfo for `", variant.variant->variant().identifier(),
        "` does not match variant index ", variant.variant_index, " in sum `",
