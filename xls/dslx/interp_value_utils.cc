...
absl::StatusOr<bool> MatchValueObservation::EqualsConstant(
    ...
    const Path& path) {
  XLS_RETURN_IF_ERROR(
      ValidateValue(constant, type, SumValidation::kDeclaredConstructors));
...
absl::StatusOr<bool> SemanticValuesEqual(const InterpValue& lhs,
                                         ...
                                         const Type& type) {
  XLS_RETURN_IF_ERROR(
      ValidateValue(lhs, type, SumValidation::kDeclaredConstructors));
  XLS_RETURN_IF_ERROR(
      ValidateValue(rhs, type, SumValidation::kDeclaredConstructors));
