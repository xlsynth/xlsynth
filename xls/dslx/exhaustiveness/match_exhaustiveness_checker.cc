...
int64_t GetLeafTypeCount(const Type& type) {
  ...
  if (type.IsSum()) {
    ...
    CHECK_OK(encoding.ForEachStoredLeafType(
        [&](const Phase1SumTypeEncoding::StoredLeafInfo& leaf) -> absl::Status {
...
FlattenedLeafTypes GetSumVariantPayloadLeafTypes(const SumType& sum_type,
                                                 std::string_view variant_name) {
  FlattenedLeafTypes result;
  const Phase1SumTypeEncoding encoding(sum_type);
  Phase1SumTypeEncoding::VariantInfo variant = encoding.GetVariant(variant_name).value();
...
NdIntervalWithEmpty PatternLeavesToInterval(
  ...
  for (int64_t i = 0; i < pattern_leaves.size(); ++i) {
    intervals.push_back(PatternToIntervalInternal(pattern_leaves[i],
                                                  leaf_types[i], type_info,
                                                  import_data));
  }
...
int64_t GetSumVariantIndex(const SumType& sum_type,
  ...
  return Phase1SumTypeEncoding(sum_type).GetVariant(constructor_name).value().variant_index;
}
...
std::vector<PatternLeaf> ExpandActiveSumPayloadMemberPatternLeaves(
    const SumTypeVariant& variant, const ConstructorPattern& constructor_pattern,
    int64_t active_index, const FileTable& file_table) {
  if (variant.is_tuple()) {
    CHECK(constructor_pattern.is_tuple());
    CHECK_EQ(constructor_pattern.positional_patterns().size(), variant.size());
    return ExpandPatternLeaves(
        *constructor_pattern.positional_patterns()[active_index],
...
void AppendSumVariantPayloadPatternLeaves(
    ...
    const ConstructorPattern* constructor_pattern, const FileTable& file_table,
    std::vector<PatternLeaf>* result) {
      ...
      Visitor{
          [&](ConstructorPattern* constructor_pattern)
            ...
            AppendSumVariantPayloadPatternLeaves(type.variants()[variant_index],
                                                constructor_pattern,
                                                file_table, &result);
            return SumVariantPayloadPattern{variant_index, std::move(result)};
          },
          [&](ColonRef* colon_ref) -> SumVariantPayloadPattern {
            int64_t variant_index =
                GetSumVariantIndex(type, colon_ref->attr());
            CHECK(type.variants()[variant_index].is_unit());
            return SumVariantPayloadPattern{variant_index, {}};
          },
          [&](const auto&) -> SumVariantPayloadPattern {
      ...
      Visitor{
          [&](ConstructorPattern* constructor_pattern)
            ...
            Phase1SumTypeEncoding::VariantInfo variant =
                encoding.GetVariant(constructor_pattern->constructor()->attr())
                    .value();
...
NdRegion MakeFullNdRegion(const FlattenedLeafTypes& leaf_types) {
  std::vector<InterpValueInterval> intervals = GetFullIntervals(leaf_types.flat);
