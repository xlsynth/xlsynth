...
using PatternLeaf = std::variant<SomeWildcard, InterpValue, NameRef*, Range*,
                                 ColonRef*, Number*>;
...
NdIntervalWithEmpty PatternLeavesToInterval(
    absl::Span<const PatternLeaf> pattern_leaves,
...
std::vector<PatternLeaf> ExpandActiveSumPayloadMemberPatternLeaves(
    const SumTypeVariant& variant,
...
void AppendSumVariantPayloadPatternLeaves(
    ...
    const FileTable& file_table, std::vector<PatternLeaf>* result) {
...
struct ExpandedSumVariantPattern {
  int64_t variant_index;
  std::vector<PatternLeaf> leaves;
};

ExpandedSumVariantPattern ExpandSumVariantPayloadPatternLeaves(
    const NameDefTree::Leaf& leaf, const SumType& type,
    const FileTable& file_table) {
  return absl::visit(
      Visitor{
          [&](SumVariantPayloadPattern* constructor_pattern)
...
std::vector<PatternLeaf> ExpandPatternLeaves(const NameDefTree& pattern,
  ...
  if (type.IsSum()) {
    CHECK(pattern.is_leaf()) << "Expected a leaf pattern for sum type, got `"
                             << pattern.ToString() << "`";
...
NdIntervalWithEmpty PatternToInterval(
    const NameDefTree& pattern, const Type& matched_type,
    absl::Span<const FlattenedLeafType> leaf_types, const TypeInfo& type_info,
    const ImportData& import_data) {
  ...
  if (impl_->matched_sum_type_ != nullptr) {
    ...
    ExpandedSumVariantPattern variant_pattern =
        ExpandSumVariantPayloadPatternLeaves(
            pattern.leaf(), *impl_->matched_sum_type_, file_table());
