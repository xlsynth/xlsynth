...
struct FlattenedLeafType {
  ...
  // Sum tags use consecutive inhabited-constructor coordinates. Keep excluded
  // declaration indices sorted for pattern conversion and witness decoding.
  std::vector<int64_t> uninhabited_variant_indices;
...
struct FlattenedLeafTypes {
  ...
  //
  // Empty sums and empty enums contribute no checker leaf to the semantic match
...
int64_t GetSumPayloadLeafTypeCount(const SumType& sum_type) {
  int64_t result = 0;
  for (const SumTypeVariant& variant : sum_type.variants()) {
    for (int64_t member_index = 0; member_index < variant.size();
         ++member_index) {
      result += GetLeafTypeCount(variant.GetMemberType(member_index));
    }
  }
  return result;
}
...
int64_t GetLeafTypeCount(const Type& type) {
  if (IsEmptyEnum(type)) {
    ...
    for (int64_t i = 0; i < struct_type->size(); ++i) {
      result += GetLeafTypeCount(struct_type->GetMemberType(i));
    }
    return result;
  } else if (auto* array_type = dynamic_cast<const ArrayType*>(&type);
             array_type != nullptr && !GetBitsLike(type).has_value()) {
    return array_type->size().GetAsInt64().value() *
           GetLeafTypeCount(array_type->element_type());
  } else if (type.IsSum()) {
    if (type.AsSum().variant_count() == 0) {
      return 0;
    } else {
      return 1 + GetSumPayloadLeafTypeCount(type.AsSum());
    }
  } else {
    return 1;
  }
  ...
  if (IsEmptyEnum(type)) {
    ...
    for (const std::unique_ptr<Type>& member : type.AsTuple().members()) {
      AppendStorageLeafTypes(*member, result);
             ...
             struct_type != nullptr) {
    for (int64_t i = 0; i < struct_type->size(); ++i) {
      AppendStorageLeafTypes(struct_type->GetMemberType(i), result);
    }
  } else if (auto* array_type = dynamic_cast<const ArrayType*>(&type);
             array_type != nullptr && !GetBitsLike(type).has_value()) {
    const int64_t size = array_type->size().GetAsInt64().value();
    for (int64_t i = 0; i < size; ++i) {
      AppendStorageLeafTypes(array_type->element_type(), result);
    }
  } else if (type.IsSum()) {
    if (type.AsSum().variant_count() != 0) {
      const SumType& sum_type = type.AsSum();
      const int64_t tag_bit_count =
          sum_type.tag_bit_count().GetAsInt64().value();
      result->owned.push_back(
          std::make_unique<BitsType>(/*is_signed=*/false, tag_bit_count));
      const int64_t tag_leaf_index = result->flat.size();
      result->flat.push_back(FlattenedLeafType{
          .type = result->owned.back().get(),
          .dense_max_value = 0,
          .uninhabited_variant_indices = {},
      ...
      for (int64_t variant_index = 0; variant_index < sum_type.variant_count();
        ...
        for (int64_t member_index = 0; member_index < variant.size();
             ++member_index) {
          AppendStorageLeafTypes(variant.GetMemberType(member_index), result);
        }
        if (!SumVariantIsInhabited(variant).value()) {
          uninhabited_variant_indices.push_back(variant_index);
        }
      }
      const int64_t uninhabited_count = uninhabited_variant_indices.size();
      const int64_t inhabited_count =
          sum_type.variant_count() - uninhabited_count;
      // Recursive appends can reallocate flat, so look up the tag by index.
      FlattenedLeafType& tag_leaf = result->flat[tag_leaf_index];
      // Contiguous coordinates avoid multiplying constructor holes across
      // independent dimensions. An uninhabited sum in an inactive payload keeps
      // one storage placeholder; an actively required empty type is handled by
      // FlattenedLeafTypes::is_empty instead.
      tag_leaf.dense_max_value = std::max<int64_t>(0, inhabited_count - 1);
      tag_leaf.uninhabited_variant_indices =
          std::move(uninhabited_variant_indices);
    }
  } else {
    std::optional<EnumValueDomain> enum_domain;
    if (type.IsEnum()) {
...
void AppendLeafTypes(const Type& type, FlattenedLeafTypes* result) {
  result->is_empty = result->is_empty || !TypeIsInhabited(type).value();
...
FlattenedLeafTypes GetSumVariantPayloadLeafTypes(
  ...
  for (int64_t member_index = 0; member_index < variant.size();
       ++member_index) {
    AppendLeafTypes(variant.GetMemberType(member_index), &result);
  }
  return result;
}
...
std::optional<InterpValueInterval> MakePointIntervalForLeafType(
  ...
  } else if (leaf_type.dense_max_value.has_value()) {
    const int64_t variant_index = value.GetBitValueUnsigned().value();
    const std::vector<int64_t>& uninhabited =
        leaf_type.uninhabited_variant_indices;
    const auto it =
        std::lower_bound(uninhabited.begin(), uninhabited.end(), variant_index);
    if (it != uninhabited.end() && *it == variant_index) {
      return std::nullopt;
    } else {
      InterpValue coordinate =
          InterpValue::MakeUBits(value.GetBitsOrDie().bit_count(),
                                 variant_index - (it - uninhabited.begin()));
      return InterpValueInterval(coordinate, coordinate);
    }
  } else {
...
int64_t GetSumVariantIndex(const SumType& sum_type,
  ...
  for (int64_t variant_index = 0; variant_index < sum_type.variant_count();
       ++variant_index) {
    if (sum_type.variants()[variant_index].variant().identifier() ==
        constructor_name) {
      return variant_index;
    }
  }
  LOG(FATAL) << "No variant `" << constructor_name << "` in sum `"
             << sum_type.ToString() << "`";
}

// Nested sums still use a checker-local declaration-order product until Phase 3
// replaces this MVP with a recursive checker. This ordinal is semantic checker
// state, not the runtime payload-slot layout or a wire discriminant.
InterpValue MakeDenseSumVariantOrdinal(const SumType& sum_type,
  ...
  if (type.IsTuple()) {
    ...
    for (int64_t i = 0; i < members.size(); ++i) {
      AppendConstantValueLeaves(member_values[i], *members[i], result);
             ...
             struct_type != nullptr) {
    const std::vector<InterpValue>& members = value.GetValuesOrDie();
    CHECK_EQ(members.size(), struct_type->size());
    for (int64_t i = 0; i < struct_type->size(); ++i) {
      AppendConstantValueLeaves(members[i], struct_type->GetMemberType(i),
                                result);
    }
  } else if (auto* array_type = dynamic_cast<const ArrayType*>(&type);
             array_type != nullptr && !GetBitsLike(type).has_value()) {
    for (const InterpValue& member : value.GetValuesOrDie()) {
      AppendConstantValueLeaves(member, array_type->element_type(), result);
    }
  } else if (type.IsSum()) {
    const SumType& sum_type = type.AsSum();
    absl::StatusOr<internal::EncodedSumView> encoded =
...
std::optional<SumTypeEncoding::VariantInfo> GetDirectUnitSumVariant(
    const ColonRef& pattern, const SumType& type, const TypeInfo& type_info) {
  std::optional<SumTypeEncoding::VariantInfo> result;
  if (!type_info.IsKnownConstExpr(&pattern)) {
    absl::StatusOr<SumTypeEncoding::VariantInfo> variant =
        SumTypeEncoding(type).GetVariant(pattern.attr());
    if (variant.ok() && variant->variant->is_unit()) {
  ...
  return absl::visit(
      Visitor{[&](SumVariantPayloadPattern* constructor_pattern)
              ...
              [&](ColonRef* colon_ref) -> ExpandedSumVariantPattern {
                std::optional<SumTypeEncoding::VariantInfo> variant =
...
std::vector<IntervalPatternLeaf> ExpandSumPatternLeaves(
  ...
  auto make_variant_pattern_leaves =
    ...
    std::vector<IntervalPatternLeaf> result;
    result.push_back(MakeDenseSumVariantOrdinal(type, active_variant_index));
    for (int64_t variant_index = 0; variant_index < type.variant_count();
         ++variant_index) {
      const SumTypeVariant& variant = type.variants()[variant_index];
      if (variant_index == active_variant_index) {
        AppendSumVariantPayloadPatternLeaves(variant, constructor_pattern,
                                             type_info, file_table, &result);
      } else {
        for (int64_t member_index = 0; member_index < variant.size();
             ++member_index) {
              ...
              [&](ColonRef* colon_ref) -> std::vector<IntervalPatternLeaf> {
                ...
                if (variant.has_value()) {
                  return make_variant_pattern_leaves(
                      variant->variant_index, /*constructor_pattern=*/nullptr);
                }
                SumConstantValue constant =
...
std::vector<IntervalPatternLeaf> ExpandPatternLeaves(
  ...
  const bool is_expanded_aggregate =
      type.IsTuple() || dynamic_cast<const StructTypeBase*>(&type) != nullptr ||
      (dynamic_cast<const ArrayType*>(&type) != nullptr &&
       !GetBitsLike(type).has_value());
  if (!is_expanded_aggregate) {
    CHECK(!std::holds_alternative<TuplePattern*>(pattern))
        << "Expected a single leaf for non-aggregate type";
  ...
  if (const auto* name_ref = std::get_if<NameRef*>(&pattern);
    ...
    CHECK(value.has_value()) << "Missing aggregate constexpr value for `"
                             << (*name_ref)->ToString() << "`";
    std::vector<IntervalPatternLeaf> result;
  ...
  for (int64_t flattened_index = 0; flattened_index < flattened.size();
    ...
    auto append_non_rest_leaf = [&]() {
      ...
      if (type_at_index.IsSum() || type_at_index.IsTuple() ||
          dynamic_cast<const StructTypeBase*>(&type_at_index) != nullptr ||
          (dynamic_cast<const ArrayType*>(&type_at_index) != nullptr &&
           !GetBitsLike(type_at_index).has_value())) {
...
NdRegion MakeFullNdRegion(const FlattenedLeafTypes& leaf_types) {
  ...
  if (leaf_types.is_empty) {
    ...
    return NdRegion::MakeFromNdInterval(NdInterval(intervals),
...
std::string FormatSampleForType(
  ...
  if (type.IsEnum()) {
    ...
    result += ")";
             ...
             struct_type != nullptr) {
    result = struct_type->struct_def_base().identifier();
    result += " {";
    for (int64_t i = 0; i < struct_type->size(); ++i) {
      result += i == 0 ? " " : ", ";
      result += struct_type->GetMemberName(i);
      result += ": ";
      result += FormatSampleForType(struct_type->GetMemberType(i), dimensions,
                                    leaf_types, leaf_index);
    }
    result += " }";
  } else if (auto* array_type = dynamic_cast<const ArrayType*>(&type);
    ...
    for (int64_t i = 0; i < size; ++i) {
      ...
      result += FormatSampleForType(array_type->element_type(), dimensions,
                                    leaf_types, leaf_index);
    }
    result += "]";
  } else if (type.IsSum()) {
    ...
    int64_t variant_index =
        dimensions[(*leaf_index)++].min().GetBitValueUnsigned().value();
    for (int64_t uninhabited_index : leaf_type.uninhabited_variant_indices) {
      if (uninhabited_index <= variant_index) {
        ++variant_index;
      } else {
        break;
      }
    ...
    for (int64_t candidate_index = 0;
         candidate_index < sum_type.variant_count(); ++candidate_index) {
      const SumTypeVariant& candidate = sum_type.variants()[candidate_index];
      for (int64_t member_index = 0; member_index < candidate.size();
           ++member_index) {
        if (candidate_index == variant_index) {
          payload_values.push_back(
              FormatSampleForType(candidate.GetMemberType(member_index),
                                  dimensions, leaf_types, leaf_index));
        } else {
          *leaf_index +=
              GetLeafTypeCount(candidate.GetMemberType(member_index));
  ...
  if (impl_->matched_type_.IsSum()) {
    ...
    for (const SumTypeVariant& variant : impl_->matched_sum_type_->variants()) {
      FlattenedLeafTypes variant_leaf_types =
          GetSumVariantPayloadLeafTypes(variant);
...
MatchExhaustivenessChecker::AddPattern(const PatternTree& pattern) {
  ...
  if (HasRuntimeDependentPatternLeaf(pattern, impl_->type_info_)) {
    ...
    std::vector<IntervalPatternLeaf> pattern_leaves = ExpandPatternLeaves(
        pattern, impl_->matched_type_, impl_->type_info_, file_table());
    NdIntervalWithEmpty this_pattern_interval = PatternLeavesToInterval(
        pattern_leaves, impl_->leaf_types_.flat, impl_->type_info_);
