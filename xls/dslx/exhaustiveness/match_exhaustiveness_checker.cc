...
FlattenedLeafTypes GetSumVariantPayloadLeafTypes(const SumType& sum_type,
  ...
  SumTypeEncoding::VariantInfo variant = encoding.GetVariant(variant_name).value();
...
int64_t GetSumVariantIndex(const SumType& sum_type,
  ...
  return SumTypeEncoding(sum_type).GetVariant(constructor_name).value().variant_index;
}
...
void AppendSumVariantPayloadPatternLeaves(
  ...
  if (variant.is_tuple()) {
    ...
    for (int64_t member_index = 0; member_index < variant.size();
         ++member_index) {
      std::vector<PatternLeaf> member_leaves = ExpandPatternLeaves(
  ...
  for (int64_t member_index = 0; member_index < variant.size();
    ...
    std::vector<PatternLeaf> member_leaves = ExpandPatternLeaves(
        *it->second, variant.GetMemberType(member_index), file_table);
    result->insert(result->end(), member_leaves.begin(), member_leaves.end());
  }
}
...
std::vector<PatternLeaf> ExpandSumPatternLeaves(const NameDefTree::Leaf& leaf,
  ...
  auto make_variant_pattern_leaves =
      [&](const SumTypeEncoding::VariantInfo& active_variant,
    ...
    CHECK_OK(encoding.VisitPayloadAssemblyOrder(
        ...
        [&](int64_t active_index) -> absl::Status {
          ...
          CHECK(it != constructor_pattern->named_patterns().end())
              << "Missing named pattern for member `" << member_name << "`";
          std::vector<PatternLeaf> member_leaves = ExpandPatternLeaves(
              *it->second, variant.GetMemberType(active_index), file_table);
          result.insert(result.end(), member_leaves.begin(),
                        member_leaves.end());
          return absl::OkStatus();
        },
