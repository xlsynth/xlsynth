...
TEST(InterpValueRangeTest, ShouldMerge) {
  auto range1 = InterpValueRange(InterpValue::MakeUBits(1, 0),
                                 InterpValue::MakeUBits(1, 0));
  auto range2 = InterpValueRange(InterpValue::MakeUBits(1, 1),
                                 InterpValue::MakeUBits(1, 1));
  EXPECT_TRUE(range1.ShouldMergeWith(range2));
}

TEST(ExhaustivenessTest, ExhaustiveBooleanRange) {
  BitsLikeProperties properties =
      BitsLikeProperties{TypeDim::CreateBool(false), TypeDim::CreateU32(1)};
  auto range = BitsValueRange::MakeSingleRange(
      properties, InterpValue::MakeUBits(1, 0), InterpValue::MakeUBits(1, 1));
...
TEST(ExhaustivenessTest, InexhaustiveBooleanRangeJustFalse) {
  BitsLikeProperties properties =
      BitsLikeProperties{TypeDim::CreateBool(false), TypeDim::CreateU32(1)};
  auto range = BitsValueRange::MakeSingleRange(
      properties, InterpValue::MakeUBits(1, 0), InterpValue::MakeUBits(1, 0));
...
TEST(ExhaustivenessTest, InexhaustiveBooleanRangeJustTrue) {
  BitsLikeProperties properties =
      BitsLikeProperties{TypeDim::CreateBool(false), TypeDim::CreateU32(1)};
  auto range = BitsValueRange::MakeSingleRange(
      properties, InterpValue::MakeUBits(1, 1), InterpValue::MakeUBits(1, 1));
  EXPECT_FALSE(range.IsExhaustive());
}
...
TEST(ExhaustivenessTest, MergeTwoInexhaustiveBooleanRangesMakesExhaustive) {
  BitsLikeProperties properties =
      BitsLikeProperties{TypeDim::CreateBool(false), TypeDim::CreateU32(1)};
  auto range1 = BitsValueRange::MakeSingleRange(
      properties, InterpValue::MakeUBits(1, 0), InterpValue::MakeUBits(1, 0));
  auto range2 = BitsValueRange::MakeSingleRange(
      properties, InterpValue::MakeUBits(1, 1), InterpValue::MakeUBits(1, 1));
  auto merged = BitsValueRange::Merge(range1, range2);
  EXPECT_EQ(merged.disjoint().size(), 1);
  LOG(INFO) << "merged: " << merged.ToString();
...
TEST(ExhaustivenessTest, ThreeBitRangeFillInMiddleLast) {
  ...
  BitsLikeProperties properties = BitsLikeProperties{
      TypeDim::CreateBool(false), TypeDim::CreateU32(kBitCount)};
  auto low_02 = BitsValueRange::MakeSingleRange(
      properties, InterpValue::MakeUBits(kBitCount, 0),
      InterpValue::MakeUBits(kBitCount, 2));
  auto high_47 = BitsValueRange::MakeSingleRange(
      properties, InterpValue::MakeUBits(kBitCount, 4),
      InterpValue::MakeUBits(kBitCount, 7));
  auto merged = BitsValueRange::Merge(low_02, high_47);
  EXPECT_EQ(merged.disjoint().size(), 2);
  LOG(INFO) << "merged: " << merged.ToString();
  EXPECT_FALSE(merged.IsExhaustive());

  auto range_with_3 = BitsValueRange::MakeSingleRange(
      properties, InterpValue::MakeUBits(kBitCount, 3),
      InterpValue::MakeUBits(kBitCount, 3));
  auto merged_with_3 = BitsValueRange::Merge(merged, range_with_3);
  EXPECT_EQ(merged_with_3.disjoint().size(), 1);
  LOG(INFO) << "merged_with_3: " << merged_with_3.ToString();
