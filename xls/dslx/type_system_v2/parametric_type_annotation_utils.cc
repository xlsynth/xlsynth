...
absl::StatusOr<const TypeAnnotation*> GetParametricFreeType(
  ...
  replacer = ChainCloneReplacers(
      ...
      [&](const AstNode* node, Module*,
        ...
        // Type aliases declared in impls are TypeRefTypeAnnotations rather
        // than TypeVariableTypeAnnotations, so replace their enclosing
        // annotation directly when the alias binding is known.
