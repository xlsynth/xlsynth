...
absl::StatusOr<SignednessAndBitCountResult> GetSignednessAndBitCount(
  ...
  if (const auto* builtin_annotation =
    ...
    if (const auto* array_annotation =
      ...
      if (const auto* inner_array_annotation =
        ...
        // If the array has 2 dimensions, let's work with the hypothesis that
        // it's an `xN[S][N]` kind of annotation. We retain the bit count, which
        // is the outer dim, and unwrap the inner array to be processed below.
        // If it turns out to be some other multi-dimensional array type that
        // does not have a signedness and bit count, we will fail below.
    ...
    return absl::NotFoundError(absl::StrCat(
        "Cannot extract signedness and bit count from annotation: ",
        annotation->ToString()));
  }
  ...
  FunctionTypeAnnotation* CreateFunctionTypeAnnotation(
      Module & module, const Function& function) {
  ...
  const ArrayTypeAnnotation* CastToNonBitsArrayTypeAnnotation(
    ...
    // If the signedness and bit count can be retrieved, then it's some flavor
    // of xN, uN, sN, etc. and not what this function is looking for. If that
    // yields an invalid argument error, then it's still one of those types but
  ...
  Expr* CreateElementCountOffset(Module & module, TypeAnnotation * lhs,
    ...
    return module.Make<Binop>(
        ...
        module.Make<Number>(lhs->span(), absl::StrCat(offset),
                            NumberKind::kOther,
                            /*type_annotation=*/nullptr),
        Span::None());
  }
  ...
  Expr* CreateRangeElementCount(Module & module, const Range* range) {
    ...
    // Cast start and end to s32 since array size type is assumed to be U32,
    // this ensures arithmetic correctness for types smaller than 32 bit, or
    // types greater than 32 bits as long as the difference fits in a U32. If
    // the difference does not fit in a U32, for example,
    // 0xFFFF,FFFF,FFFF,FFFF..0x0000111100001111, it is silently truncated to
    // U32, and this needed to be checked at const evaluation of the range.
    Expr* start = module.Make<Cast>(span, range->start(),
                                    CreateS32Annotation(module, span));
    Expr* end = module.Make<Cast>(span, range->end(),
                                  CreateS32Annotation(module, span));
  ...
  const BuiltinTypeAnnotation* CastToTokenType(
      const TypeAnnotation* annotation) {
  ...
  bool IsBitsLikeFragment(const TypeAnnotation* annotation) {
    ...
    if (const auto* array =
            dynamic_cast<const ArrayTypeAnnotation*>(annotation)) {
      // The immediate element type of a complete xN annotation would be a
      // nested array type and not xN.
