...
const X = MaybePoint::Point { x: u32:1, y: u32:2, z: u32:3 };
      ...
      TypecheckFails(
          HasSubstr("Constructor `Point` has no member `z`")));
}
...
TEST(TypecheckV2Test,
     ImplicitSemanticSumRejectsTagTypeAnnotationInPhase1) {
  EXPECT_THAT(
      R"(
enum MaybeU32 : u3 {
  None,
  Some(u32),
      ...
      TypecheckFails(HasSubstr(
          "Phase 1 semantic sum `MaybeU32` may specify a tag type only when "
          "every variant has an explicit discriminant.")));
}
