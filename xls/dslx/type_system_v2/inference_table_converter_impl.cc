...
absl::Status ValidateSemanticSumDiscriminants(
  ...
  for (const SumVariant* variant : sum_def.variants()) {
    std::optional<Expr*> discriminant = variant->discriminant();
    ...
    if (!seen_values.insert(bits.ToDebugString()).second) {
      return TypeInferenceErrorStatus(
          ...
          absl::Substitute("Semantic sum `$0` has duplicate discriminant `$1`.",
...
GetSemanticSumTagLayout(const SumDef& sum_def, TypeInfo* type_info,
                        const std::vector<SumTypeVariant>& variants,
                        const FileTable& file_table) {
  const bool has_explicit_discriminants =
      absl::c_any_of(sum_def.variants(), [](const SumVariant* variant) {
        return variant->discriminant().has_value();
  ...
  if (sum_def.tag_type_annotation() != nullptr) {
    std::optional<Type*> annotation_type =
        type_info->GetItem(sum_def.tag_type_annotation());
    XLS_RET_CHECK(annotation_type.has_value());
    XLS_RET_CHECK(annotation_type.value() != nullptr);
    XLS_ASSIGN_OR_RETURN(const Type* tag_type,
                         UnwrapMetaType(*annotation_type.value()));
    ...
    if (!has_explicit_discriminants && !variants.empty() &&
        Bits::MinBitCountUnsigned(variants.size() - 1) > tag_bit_count) {
      return TypeInferenceErrorStatusForAnnotation(
          sum_def.tag_type_annotation()->span(), sum_def.tag_type_annotation(),
          absl::Substitute(
              "Semantic sum `$0` needs at least $1 tag bits for $2 implicit "
              "constructors, but tag type `$3` has only $4 bits.",
              sum_def.identifier(),
              Bits::MinBitCountUnsigned(variants.size() - 1), variants.size(),
    ...
    if (!has_explicit_discriminants) {
      ...
      discriminants.reserve(variants.size());
  ...
  for (const SumVariant* variant : sum_def.variants()) {
    std::optional<Expr*> discriminant = variant->discriminant();
    XLS_RET_CHECK(discriminant.has_value());
    XLS_ASSIGN_OR_RETURN(InterpValue discriminant_value,
                         type_info->GetConstExpr(*discriminant));
    discriminants.push_back(std::move(discriminant_value));
  }
  XLS_RET_CHECK(!discriminants.empty());
  XLS_ASSIGN_OR_RETURN(int64_t tag_bit_count,
                       discriminants.front().GetBitCount());
  return std::make_pair(TypeDim::CreateU32(tag_bit_count),
                        std::move(discriminants));
}
...
class InferenceTableConverterImpl : public InferenceTableConverter,
 ...
 public:
  ...
  absl::Status ConvertSumConstructorInvocation(
    ...
    const auto* function_type =
        (*signature)->AsAnnotation<FunctionTypeAnnotation>();
    if (invocation->args().size() != function_type->param_types().size()) {
      return ArgCountMismatchErrorStatus(
          invocation->span(),
          absl::Substitute("Expected $0 argument(s) but got $1.",
  ...
  absl::StatusOr<std::unique_ptr<Type>> Concretize(
    ...
    if (sum_ref.has_value()) {
      ...
      XLS_ASSIGN_OR_RETURN(auto tag_layout,
                           GetSemanticSumTagLayout(*sum_def, sum_type_info,
                                                   variants, file_table_));
      std::unique_ptr<Type> type = std::make_unique<SumType>(
          *sum_def, std::move(variants),
          absl::flat_hash_map<std::string, TypeDim>{},
          std::move(tag_layout.first), std::move(tag_layout.second));
      if (!sum_def->IsParametric()) {
 ...
 private:
  ...
  absl::StatusOr<const TypeAnnotation*> InstantiateParametricSum(
      ...
      const std::vector<InterpValue>& explicit_parametrics,
      std::optional<const Expr*> instantiator_node) override {
    ...
    ParametricBindings bindings(sum_def.parametric_bindings());
    absl::flat_hash_map<std::string, ExprOrType> resolved_parametrics;
    auto set_value = [&](const ParametricBinding* binding,
                         InterpValue value) -> absl::Status {
      if (binding->type_annotation()->IsAnnotation<GenericTypeAnnotation>()) {
        ...
        instance_type_info->SetItem(binding->name_def(), *binding_type);
        instance_type_info->NoteConstExpr(binding->name_def(), value);
        const Span& local_span = binding->owner() == &module ? binding->span()
                                 : instantiator_node.has_value()
                                     ? (*instantiator_node)->span()
                                     : span;
        XLS_ASSIGN_OR_RETURN(
            Number * value_expr,
            MakeTypeCheckedNumber(module, table_, local_span, value,
                                  binding->type_annotation()));
        resolved_parametrics.emplace(binding->identifier(), value_expr);
      }
      return absl::OkStatus();
    ...
    std::vector<Expr*> actual_member_exprs;
    if (instantiator_node.has_value()) {
      if (const auto* sum_instance =
              dynamic_cast<const SumInstance*>(*instantiator_node);
          sum_instance != nullptr) {
          ...
          for (const StructMemberNode* member : (*variant)->struct_members()) {
            ...
            actual_member_exprs.push_back(
                actual_member_exprs_by_name.at(member->name()));
                     ...
                     dynamic_cast<const Invocation*>(*instantiator_node);
                 invocation != nullptr) {
        const auto* constructor =
            dynamic_cast<const ColonRef*>(invocation->callee());
        XLS_RET_CHECK(constructor != nullptr);
        std::optional<const SumVariant*> variant =
            sum_def.GetVariant(constructor->attr());
        XLS_RET_CHECK(variant.has_value());
        XLS_RET_CHECK((*variant)->is_tuple());
        XLS_RET_CHECK_EQ((*variant)->tuple_members().size(),
                         invocation->args().size());
        formal_member_types.insert(formal_member_types.end(),
                                   (*variant)->tuple_members().begin(),
                                   (*variant)->tuple_members().end());
        actual_member_exprs.insert(actual_member_exprs.end(),
                                   invocation->args().begin(),
                                   invocation->args().end());
      } else if (const auto* struct_instance =
                     dynamic_cast<const StructInstanceBase*>(
                         *instantiator_node);
                 struct_instance != nullptr) {
        XLS_ASSIGN_OR_RETURN(
            std::optional<SumConstructorRef> constructor_ref,
            ResolveSumConstructor(struct_instance->struct_ref(), import_data_));
        XLS_RET_CHECK(constructor_ref.has_value());
        XLS_RET_CHECK_EQ(constructor_ref->sum_ref.def, &sum_def);
        XLS_RET_CHECK(constructor_ref->variant->is_struct());
        absl::flat_hash_map<std::string, Expr*> actual_member_exprs_by_name;
        for (const auto& [name, expr] : struct_instance->members()) {
          actual_member_exprs_by_name.emplace(name, expr);
        }
        XLS_RET_CHECK_EQ(constructor_ref->variant->struct_members().size(),
                         actual_member_exprs_by_name.size());
        for (const StructMemberNode* member :
             constructor_ref->variant->struct_members()) {
          formal_member_types.push_back(member->type());
          actual_member_exprs.push_back(
              actual_member_exprs_by_name.at(member->name()));
        }
    ...
    auto infer_pending_implicit_parametrics = [&]() -> absl::Status {
      VLOG(6) << "Infer implicit semantic-sum parametrics: "
              << implicit_parametrics.size();
      if (implicit_parametrics.empty()) {
        return absl::OkStatus();
      } else if (!instantiator_node.has_value() ||
                 formal_member_types.empty()) {
        return TypeInferenceErrorStatus(
            span, /*type=*/nullptr,
            absl::Substitute(
        ...
        XLS_ASSIGN_OR_RETURN(
            ...
            InferImplicitParametrics(
                span, parent_context, /*target_context=*/std::nullopt,
                sum_def.parametric_bindings(), implicit_parametrics,
                formal_member_types, actual_member_exprs, instance_type_info,
                actual_arg_ti, [&](const Expr* actual_arg) {
                  if (actual_arg->kind() == AstNodeKind::kInvocation) {
                    return ConvertSubtree(actual_arg, std::nullopt,
    ...
    std::vector<ExprOrType> resolved_parametrics_vector;
    resolved_parametrics_vector.reserve(sum_def.parametric_bindings().size());
    for (const ParametricBinding* binding : sum_def.parametric_bindings()) {
      resolved_parametrics_vector.push_back(
          resolved_parametrics.at(binding->identifier()));
    }
