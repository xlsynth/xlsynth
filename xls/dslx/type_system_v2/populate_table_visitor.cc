...
class PopulateInferenceTableVisitor : public PopulateTableVisitor,
 ...
 public:
  ...
  absl::Status HandleColonRef(const ColonRef* node) override {
    ...
    // `S<parametrics>::CONSTANT` or `S<parametrics>::static_fn`. We produce a
  ...
  absl::Status HandleMatch(const Match* node) override {
    ...
    absl::flat_hash_map<std::string, const PatternTree*> seen_patterns;
    absl::flat_hash_map<const NameDef*, const PatternTree*> seen_enum_members;
    for (MatchArm* arm : node->arms()) {
      ...
      for (const PatternTree& pattern : arm->patterns()) {
        if (auto [it, inserted] =
                seen_patterns.try_emplace(PatternToString(pattern), &pattern);
            !inserted) {
          return duplicate_pattern_error(GetPatternSpan(*it->second),
                                         GetPatternSpan(pattern), it->first);
        }
