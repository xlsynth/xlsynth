...
absl::flat_hash_map<const NameDef*, NameDef*> BuildNameDefMap(
  ...
  for (const auto& [old_node, new_node] : old_to_new) {
    if (const auto* old_name_def = dynamic_cast<const NameDef*>(old_node)) {
      name_def_map.emplace(old_name_def, absl::down_cast<NameDef*>(new_node));
    }
  }
  return name_def_map;
}
...
absl::StatusOr<std::optional<AstNode*>> RebindLocalTypeRef(
  ...
  std::string identifier = absl::visit(
      Visitor{[](const auto* def) { return def->identifier(); }}, name_def);
...
absl::StatusOr<ColonRef*> CloneColonRefIntoModule(
    ...
    const absl::flat_hash_map<const AstNode*, AstNode*>& old_to_new) {
  XLS_ASSIGN_OR_RETURN(
      Expr * expr, CloneExprIntoModule(colon_ref, target_module, old_to_new));
...
bool IsPatternContext(const AstNode* node) {
  for (const AstNode* current = node->parent(); current != nullptr;
    ...
    if (dynamic_cast<const TuplePattern*>(current) != nullptr ||
        dynamic_cast<const SumVariantPayloadPattern*>(current) != nullptr) {
      return true;
    } else if (const auto* arm = dynamic_cast<const MatchArm*>(current)) {
      for (const PatternTree& pattern : arm->patterns()) {
        if (ToAstNode(pattern) == node) {
          return true;
        }
      }
      return false;
...
absl::StatusOr<std::optional<std::unique_ptr<Module>>> CanonicalizeSumInstances(
  ...
  CloneReplacer replacer =
    ...
    if (const auto* invocation = dynamic_cast<const Invocation*>(node)) {
      ...
      return target_module->Make<SumInstance>(
          ...
          SumInstance::PayloadShape::kTuple, std::move(args),
          std::vector<SumInstance::StructPayloadFieldArg>{},
          invocation->in_parens());
      ...
      std::vector<SumInstance::StructPayloadFieldArg> struct_payload_field_args;
      struct_payload_field_args.reserve(struct_instance->members().size());
      for (const auto& [name, arg] : struct_instance->members()) {
        XLS_ASSIGN_OR_RETURN(
            Expr * cloned_arg,
            CloneExprIntoModule(arg, target_module, old_to_new));
        struct_payload_field_args.push_back(std::make_pair(name, cloned_arg));
      }
      changed = true;
      return target_module->Make<SumInstance>(
          struct_instance->span(), cloned_constructor,
          SumInstance::PayloadShape::kStruct, std::vector<Expr*>{},
          std::move(struct_payload_field_args), struct_instance->in_parens());
      ...
      if ((dynamic_cast<const SumInstance*>(parent) != nullptr &&
           absl::down_cast<const SumInstance*>(parent)->constructor_ref() ==
               constructor) ||
      ...
      return target_module->Make<SumInstance>(
          ...
          SumInstance::PayloadShape::kUnit, std::vector<Expr*>{},
          std::vector<SumInstance::StructPayloadFieldArg>{},
          constructor->in_parens());
    } else {
      return std::nullopt;
    }
  };
  ...
  XLS_RETURN_IF_ERROR(VerifyClone(&module, cloned.get(), *module.file_table()));
  if (!changed) {
    return std::nullopt;
  }
  while (true) {
