...
Span SelectSpanForGeneratedNode(const Module& module,
  ...
  for (const Span& span : candidates) {
    if (!span.HasFile() || !module.fs_path().has_value()) {
      return span;
    } else {
      std::string_view filename = span.GetFilename(*module.file_table());
      if (absl::StartsWith(filename, "<specialization:") ||
          filename == module.fs_path()->generic_string()) {
        return span;
      }
    }
  }
