...
absl::StatusOr<BValue> FunctionConverter::BuildEqByType(
  ...
  if (auto* sum_type = dynamic_cast<const SumType*>(&type);
    ...
    BValue payload_eq =
        function_builder_->Select(lhs_tag, case_payload_eqs, default_payload_eq,
                                  loc);
...
absl::StatusOr<BValue> FunctionConverter::BuildPhase1WellFormedPredicateByType(
  ...
  if (auto* sum_type = dynamic_cast<const SumType*>(&type);
        ...
        [&](const Phase1SumTypeEncoding::VariantInfo& variant) -> absl::Status {
          ...
          XLS_RETURN_IF_ERROR(encoding.ForEachActivePayloadSlot(
              ...
              [&](int64_t payload_slot_index, int64_t active_index,
                ...
                static_cast<void>(active_index);
                XLS_ASSIGN_OR_RETURN(
                    BValue slot_is_well_formed,
                    BuildPhase1WellFormedPredicateByType(
                        slot_type,
                        function_builder_->TupleIndex(payload,
                                                      payload_slot_index, loc),
                        loc));
                case_payload_ok = function_builder_->And(case_payload_ok,
                                                         slot_is_well_formed,
                                                         loc);
                return absl::OkStatus();
              }));
    ...
    BValue result = function_builder_->Eq(
        value,
        function_builder_->Literal(Value(declared_members.front()), loc), loc);
    for (size_t i = 1; i < declared_members.size(); ++i) {
      BValue member_eq = function_builder_->Eq(
          value,
          function_builder_->Literal(Value(declared_members.at(i)), loc), loc);
...
absl::Status FunctionConverter::HandleEq(const Binop* node, BValue lhs,
  ...
  return DefWithStatus(node,
                       [&](const SourceInfo& loc) -> absl::StatusOr<BValue> {
                         XLS_RETURN_IF_ERROR(
                             AssertPhase1SemanticSumValueIsWellFormed(
                                 *type, lhs, loc, node->span(), "equality",
                                 "equality"));
                         XLS_RETURN_IF_ERROR(
                             AssertPhase1SemanticSumValueIsWellFormed(
                                 *type, rhs, loc, node->span(), "equality",
                                 "equality"));
...
absl::Status FunctionConverter::HandleNe(const Binop* node, BValue lhs,
  ...
  return DefWithStatus(node,
                       [&](const SourceInfo& loc) -> absl::StatusOr<BValue> {
                         XLS_RETURN_IF_ERROR(
                             AssertPhase1SemanticSumValueIsWellFormed(
                                 *type, lhs, loc, node->span(), "inequality",
                                 "inequality"));
                         XLS_RETURN_IF_ERROR(
                             AssertPhase1SemanticSumValueIsWellFormed(
                                 *type, rhs, loc, node->span(), "inequality",
                                 "inequality"));
...
absl::StatusOr<BValue> FunctionConverter::HandleConstructorPattern(
  ...
  auto append_condition = [&](NameDefTree* subpattern,
                              ...
                              const Type& payload_slot_type)
      -> absl::Status {
    BValue slot_value =
        function_builder_->TupleIndex(payload, payload_slot_index, loc);
    XLS_ASSIGN_OR_RETURN(
        BValue condition,
        HandleMatcher(subpattern, slot_value, payload_slot_type));
  ...
  if (pattern->is_tuple()) {
    ...
    XLS_RETURN_IF_ERROR(encoding.ForEachActivePayloadSlot(
        ...
        [&](int64_t payload_slot_index, int64_t active_index,
          ...
          return append_condition(pattern->positional_patterns().at(active_index),
...
absl::Status FunctionConverter::HandleSumConstructorInvocation(
  ...
  XLS_ASSIGN_OR_RETURN(int64_t tag_bit_count, encoding.tag_bit_count());
  Def(node, [this, &payload_slots, variant, tag_bit_count](
                const SourceInfo& loc) {
...
absl::Status FunctionConverter::HandleSumInstance(const SumInstance* node) {
  ...
  if (node->is_unit()) {
    XLS_ASSIGN_OR_RETURN(
        InterpValue value,
        CreateSumValue(sum_type, node->constructor()->attr(), {}));
  ...
  const Phase1SumTypeEncoding encoding(sum_type);
  XLS_ASSIGN_OR_RETURN(Phase1SumTypeEncoding::VariantInfo variant,
                       encoding.GetVariant(node->constructor()->attr()));

  std::vector<BValue> payload_slots;
  payload_slots.reserve(encoding.payload_slot_count());
  absl::flat_hash_map<std::string, Expr*> members_by_name;
  for (const auto& [name, value] : node->named_args()) {
  ...
  XLS_RETURN_IF_ERROR(encoding.VisitPayloadAssemblyOrder(
      ...
      [&](int64_t active_index) -> absl::Status {
        Expr* value = node->is_tuple()
                          ? node->positional_args().at(active_index)
