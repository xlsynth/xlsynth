...
absl::StatusOr<BValue> FunctionConverter::BuildEqByType(
    const Type& type, BValue lhs, BValue rhs, const SourceInfo& loc) {
  if (auto* sum_type = dynamic_cast<const SumType*>(&type);
    ...
    for (const SumTypeVariant& variant : sum_type->variants()) {
      XLS_ASSIGN_OR_RETURN(SumTypeVariantLayout layout,
                           GetSumTypeVariantLayout(*sum_type,
                                                   variant.variant().identifier()));
      BValue case_matches = function_builder_->Eq(
          lhs_tag,
          function_builder_->Literal(
              UBits(layout.variant_index, tag_bit_count), loc),
          loc);
      BValue case_payload_eq = function_builder_->Literal(UBits(1, 1), loc);
      for (int64_t i = 0; i < layout.payload_size; ++i) {
        int64_t payload_slot_index = layout.payload_start + i;
        XLS_ASSIGN_OR_RETURN(
            BValue member_eq,
            BuildEqByType(*payload_slot_types.at(payload_slot_index),
                          function_builder_->TupleIndex(lhs_payload,
                                                        payload_slot_index, loc),
                          function_builder_->TupleIndex(rhs_payload,
                                                        payload_slot_index, loc),
                          loc));
        case_payload_eq =
  ...
  if (auto* tuple_type = dynamic_cast<const TupleType*>(&type);
    ...
    for (int64_t i = 0; i < tuple_type->size(); ++i) {
      XLS_ASSIGN_OR_RETURN(
          BValue member_eq,
          BuildEqByType(tuple_type->GetMemberType(i),
                        function_builder_->TupleIndex(lhs, i, loc),
                        function_builder_->TupleIndex(rhs, i, loc), loc));
      result = function_builder_->And(result, member_eq, loc);
    }
    return result;
  }
  if (auto* struct_type = dynamic_cast<const StructTypeBase*>(&type);
    ...
    for (int64_t i = 0; i < struct_type->size(); ++i) {
      XLS_ASSIGN_OR_RETURN(
          BValue member_eq,
          BuildEqByType(struct_type->GetMemberType(i),
                        function_builder_->TupleIndex(lhs, i, loc),
                        function_builder_->TupleIndex(rhs, i, loc), loc));
      result = function_builder_->And(result, member_eq, loc);
    }
  ...
  if (auto* array_type = dynamic_cast<const ArrayType*>(&type);
    ...
    for (int64_t i = 0; i < size; ++i) {
      ...
      XLS_ASSIGN_OR_RETURN(
          BValue element_eq,
          BuildEqByType(array_type->element_type(),
                        function_builder_->ArrayIndex(lhs, {index}, loc),
                        function_builder_->ArrayIndex(rhs, {index}, loc), loc));
      result = function_builder_->And(result, element_eq, loc);
    }
    return result;
  }
  return function_builder_->Eq(lhs, rhs, loc);
...
absl::StatusOr<BValue> FunctionConverter::HandleConstructorPattern(
    NameDefTree* matcher, const ConstructorPattern* pattern,
    const BValue& matched_value, const SumType& matched_type) {
  XLS_ASSIGN_OR_RETURN(
      SumTypeVariantLayout layout,
      GetSumTypeVariantLayout(matched_type, pattern->constructor()->attr()));
  const std::vector<const Type*> payload_slot_types =
      GetSumPayloadSlotTypes(matched_type);
  XLS_ASSIGN_OR_RETURN(int64_t tag_bit_count,
                       matched_type.tag_bit_count().GetAsInt64());
  ...
  auto append_condition = [&](NameDefTree* subpattern,
    ...
    XLS_ASSIGN_OR_RETURN(
        BValue condition,
        HandleMatcher(subpattern, slot_value,
                      *payload_slot_types.at(payload_slot_index)));
    result = function_builder_->And(result, condition, loc);
    return absl::OkStatus();
  };

  if (pattern->is_tuple()) {
    XLS_RET_CHECK_EQ(pattern->positional_patterns().size(), layout.payload_size);
    for (int64_t i = 0; i < layout.payload_size; ++i) {
      XLS_RETURN_IF_ERROR(append_condition(pattern->positional_patterns().at(i),
                                           layout.payload_start + i));
    }
  } else if (pattern->is_struct()) {
    const SumTypeVariant& variant = matched_type.variants().at(layout.variant_index);
    ...
    for (int64_t i = 0; i < variant.size(); ++i) {
      XLS_RETURN_IF_ERROR(append_condition(
          named_patterns.at(variant.GetMemberName(i)),
          layout.payload_start + i));
    }
  }
...
absl::StatusOr<BValue> FunctionConverter::HandleMatcher(
  ...
  if (matcher->is_leaf()) {
    ...
    auto equality = [&](const BValue& to_match) -> absl::StatusOr<BValue> {
      if (!TypeContainsSum(matched_type)) {
      ...
      XLS_ASSIGN_OR_RETURN(BValue result,
                           BuildEqByType(matched_type, to_match, matched_value,
    ...
    return absl::visit(
        Visitor{
            ...
            [&](ConstructorPattern* pattern) -> absl::StatusOr<BValue> {
              ...
              return HandleConstructorPattern(matcher, pattern, matched_value,
                                              *sum_type);
            },
...
absl::Status FunctionConverter::HandleSumConstructorInvocation(
  ...
  XLS_ASSIGN_OR_RETURN(
      SumTypeVariantLayout layout,
      GetSumTypeVariantLayout(sum_type, constructor_ref->attr()));
  XLS_RET_CHECK_EQ(node->args().size(), layout.payload_size);

  const std::vector<const Type*> payload_slot_types =
      GetSumPayloadSlotTypes(sum_type);
  std::vector<BValue> payload_slots;
  payload_slots.reserve(payload_slot_types.size());
  int64_t active_index = 0;
  for (int64_t slot_index = 0; slot_index < payload_slot_types.size();
       ++slot_index) {
    if (slot_index >= layout.payload_start &&
        slot_index < layout.payload_start + layout.payload_size) {
  ...
  XLS_ASSIGN_OR_RETURN(int64_t tag_bit_count,
                       sum_type.tag_bit_count().GetAsInt64());
  Def(node, [this, &payload_slots, layout, tag_bit_count](
                const SourceInfo& loc) {
    BValue payload = function_builder_->Tuple(payload_slots, loc);
    BValue tag =
        function_builder_->Literal(UBits(layout.variant_index, tag_bit_count),
                                   loc);
...
absl::StatusOr<ChannelInterface*> FunctionConverter::ChannelToInterface(
  ...
  return builder_ptr->GetSendChannelInterface(channel->name());
}
...
absl::Status FunctionConverter::HandleSumStructInstance(
  ...
  const std::vector<const Type*> payload_slot_types =
      GetSumPayloadSlotTypes(sum_type);
  const SumTypeVariant& variant = sum_type.variants().at(layout.variant_index);
  ...
  for (int64_t slot_index = 0; slot_index < payload_slot_types.size();
    ...
    if (slot_index >= layout.payload_start &&
        slot_index < layout.payload_start + layout.payload_size) {
      Expr* value = members_by_name.at(std::string(variant.GetMemberName(active_index++)));
