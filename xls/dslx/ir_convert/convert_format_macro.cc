...
absl::Status FlattenSum(const ValueFormatDescriptor& sfd, const BValue& v,
                        ConvertContext& ctx) {
  // Phase 1 note: this is intentionally a temporary rendering. Trace format
  // steps are static, so the current IR path cannot choose a variant-specific
  // literal name or payload shape at runtime. Use a stable storage-oriented
  // rendering instead: sum name, numeric tag, and the flattened payload slots.
  // Phase 2 sum formatting is expected to refine this to constructor-aware
  // output once trace lowering can select among variant-specific formats.
