...
absl::StatusOr<Match*> LowerIfLetConditional(Conditional* conditional,
  ...
  Span first_arm_span(GetPatternSpan(*conditional->if_let_pattern()).start(),
                      conditional->consequent()->span().limit());
  MatchArm* first_arm = module->Make<MatchArm>(
      first_arm_span, std::vector<PatternTree>{*conditional->if_let_pattern()},
      conditional->consequent());

  Span wildcard_span(alternate->span().start(), alternate->span().start());
  WildcardPattern* wildcard = module->Make<WildcardPattern>(wildcard_span);
  ...
  return module->Make<Match>(conditional->span(), conditional->test(),
