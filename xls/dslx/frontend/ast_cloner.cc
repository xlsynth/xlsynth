...
class AstCloner : public AstNodeVisitor {
 public:
  ...
  absl::Status HandleSumVariantPayloadPattern(
    ...
    std::vector<PatternTree> new_tuple_payload_patterns;
    new_tuple_payload_patterns.reserve(n->tuple_payload_patterns().size());
    for (const PatternTree& pattern : n->tuple_payload_patterns()) {
      new_tuple_payload_patterns.push_back(ClonePattern(pattern));
    }
    ...
    for (const auto& [name, pattern] : n->struct_payload_field_patterns()) {
      new_struct_payload_field_patterns.push_back(
          std::make_pair(name, ClonePattern(pattern)));
    }
