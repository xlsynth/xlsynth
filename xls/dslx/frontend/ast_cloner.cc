...
class AstCloner : public AstNodeVisitor {
 public:
  ...
  absl::Status HandleNameRef(const NameRef* n) override {
    ...
    if (std::holds_alternative<BuiltinNameDef*>(n->name_def())) {
      auto* builtin_name_def = std::get<BuiltinNameDef*>(n->name_def());
      new_name_def =
          module(n)->GetOrCreateBuiltinNameDef(builtin_name_def->identifier());
    } else {
    ...
    old_to_new_[n] = module(n)->Make<NameRef>(
        n->span(), n->identifier(), new_name_def, n->in_parens());
    return absl::OkStatus();
  }
