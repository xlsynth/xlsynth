...
TEST(FunctionSpecializerTest, SpecializedParametersRebindNameRefs) {
  ...
  XLS_ASSERT_OK_AND_ASSIGN(
      Function * specialized,
      InsertFunctionSpecialization(passthrough_fn, env, "passthrough_N8"));
...
fn twice<N: u32>(x: bits[N]) -> bits[N] {
  ...
  XLS_ASSERT_OK_AND_ASSIGN(
      Function * specialized,
      InsertFunctionSpecialization(add_one, env, "add_one_N4"));
...
fn select_poly<N: u32>(polys: uN[6][N], selector: uN[N]) -> uN[6] {
  ...
  XLS_ASSERT_OK_AND_ASSIGN(
      TypecheckedModule typechecked,
      ParseAndTypecheck(kProgram, "invocation_test.x", "invocation_test",
                        import_data.get()));
  ...
  XLS_ASSERT_OK_AND_ASSIGN(
      Function * specialized,
      InsertFunctionSpecialization(select_poly, env, "select_poly_34"));
  ...
  std::filesystem::path module_path =
      module->fs_path().value_or(std::filesystem::path("invocation_test.x"));
...
fn select_poly<N: u32>(polys: uN[6][N], selector: uN[N]) -> uN[6] {
  ...
  XLS_ASSERT_OK_AND_ASSIGN(
      TypecheckedModule typechecked,
      ParseAndTypecheck(kProgram, "rewrite_test.x", "rewrite_test",
                        import_data.get()));
  ...
  XLS_ASSERT_OK_AND_ASSIGN(
      Function * specialized,
      InsertFunctionSpecialization(select_poly, env, "select_poly_12"));
  ASSERT_NE(specialized, nullptr);
