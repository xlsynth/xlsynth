...
// A pattern is represented directly by its concrete syntax node. Parenthesized
// patterns and semantic-sum constructor payloads contain nested patterns.
using PatternLeaf =
    std::variant<NameDef*, NameRef*, WildcardPattern*, Number*, ColonRef*,
                 SumVariantPayloadPattern*, Range*, RestOfTuple*>;
using PatternTree = std::variant<NameDef*, NameRef*, WildcardPattern*, Number*,
                                 ColonRef*, SumVariantPayloadPattern*, Range*,
                                 RestOfTuple*, TuplePattern*>;
...
class TypeRefTypeAnnotation : public TypeAnnotation {
 public:
  ...
  TypeRefTypeAnnotation(
      ...
      std::optional<const Expr*> sum_instantiator = std::nullopt);
  ...
  std::optional<const Expr*> sum_instantiator() const {
 ...
 private:
  ...
  std::optional<const Expr*> sum_instantiator_;
};
...
class SumVariantPayloadPattern : public AstNode {
 public:
  ...
  using StructPayloadFieldPattern = std::pair<std::string, PatternTree>;
  ...
  const std::vector<PatternTree>& tuple_payload_patterns() const {
    return tuple_payload_patterns_;
  }
  const std::vector<StructPayloadFieldPattern>& struct_payload_field_patterns()
      const {
    return struct_payload_field_patterns_;
  }
 ...
 private:
  ...
  std::vector<PatternTree> tuple_payload_patterns_;
  std::vector<StructPayloadFieldPattern> struct_payload_field_patterns_;
};
...
class FormatMacro : public Expr {
 public:
  ...
  std::optional<Expr*> condition() const { return condition_; }
