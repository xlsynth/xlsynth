...
TEST_F(FunctionFmtTest, SimpleConstForOneStatementNoTypeAnnotation) {
  const std::string_view original =
      "fn f(x:u32)->u32{const for(i,accum)in u32:0..u32:4{accum+i}(x)}";
  XLS_ASSERT_OK_AND_ASSIGN(std::string got, DoFmt(original));
  const std::string_view want =
      R"(fn f(x: u32) -> u32 {
    const for (i, accum) in u32:0..u32:4 {
        accum + i
    }(x)
})";
  EXPECT_EQ(got, want);
...
TEST_F(FunctionFmtTest, SimpleConstForOneStatementTypeAnnotated) {
  const std::string_view original =
      "fn f(x:u32)->u32{const for(i,accum):(u32,u32)in "
...
TEST_F(FunctionFmtTest, SimpleConstForLetBinding) {
  const std::string_view original =
      "fn f(x:u32)->u32{let y=const for(i,accum):(u32,u32)in "
      "u32:0..u32:4{accum+i}(x);y}";
  XLS_ASSERT_OK_AND_ASSIGN(std::string got, DoFmt(original));
  const std::string_view want =
      R"(fn f(x: u32) -> u32 {
    let y = const for (i, accum): (u32, u32) in u32:0..u32:4 {
        accum + i
    }(x);
    y
})";
...
TEST_F(FunctionFmtTest, LambdaWithImplicitReturn) {
  const std::string_view original =
      R"(fn f() -> u32 {
...
TEST_F(ModuleFmtTest, SemanticSumTuplePayloadComments) {
  DoFmt(
    ...
    Some(
        // Before the first tuple member.
        u32, // Before the second tuple member.
        u64, // After the final tuple member.
    ),
    Empty(
        // Comment in an empty tuple payload.
...
TEST_F(ModuleFmtTest, FuzzTestAttributeSingleArg) {
  DoFmt(R"(#[fuzz_test(domains=`u32:0..1`)]
fn f(x: u32) {
    ()
}
)");
...
TEST_F(ModuleFmtTest, FuzzTestAttributeLong) {
  DoFmt(
      R"(#[fuzz_test(domains=`u32:0..1, [u32:0, u32:10], u32:42, u32:100..200, u32:300..400`)]
