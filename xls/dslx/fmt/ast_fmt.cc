...
DocRef Fmt(const FormatMacro& n, Comments& comments, DocArena& arena) {
  ...
  if (n.condition().has_value()) {
    pieces.push_back(Fmt(**n.condition(), comments, arena));
    pieces.push_back(arena.comma());
    pieces.push_back(arena.break1());
  }
...
DocRef Fmt(const Range& n, Comments& comments, DocArena& arena) {
  if (n.inclusive_end()) {
    return ConcatNGroup(arena, {Fmt(*n.start(), comments, arena),
                                arena.break0(), arena.dot_dot(), arena.equals(),
...
DocRef FmtBlockedExprLeader(const Expr& e, Comments& comments,
  ...
  switch (e.kind()) {
    ...
    case AstNodeKind::kFor: {
      ...
      DocRef names_ref = Fmt(*n.names(), comments, arena);
      return FmtForLoopBaseLeader(Keyword::kFor, names_ref, n, comments, arena,
                                  /*is_const_for=*/false);
    }
  ...
  if (!comments.HasComments(payload_span)) {
    ...
    for (size_t i = 0; i < variant.tuple_members().size(); ++i) {
      ...
      const std::vector<const CommentData*> comment_items =
          comments.GetComments(comments_span);
      if (std::optional<DocRef> comments_doc =
