...
absl::Status BytecodeInterpreter::InitFrame(BytecodeFunction* bf,
  ...
  VLOG(100) << absl::StreamFormat("BytecodeInterpreter::InitFrame: type_info: %p", type_info);
...
absl::Status BytecodeInterpreter::EvalCall(const Bytecode& bytecode) {
  ...
  VLOG(100) << absl::StreamFormat("BytecodeInterpreter::EvalCall: new frame: %s", frames_.back().ToString(file_table()));
  return absl::OkStatus();
}
