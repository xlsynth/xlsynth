...
bool ValueFormatDescriptorContainsSum(const ValueFormatDescriptor& descriptor) {
  if (descriptor.IsSum()) {
    return true;
  } else if (descriptor.IsArray()) {
    return ValueFormatDescriptorContainsSum(descriptor.array_element_format());
  } else if (descriptor.IsTuple()) {
    return absl::c_any_of(descriptor.tuple_elements(),
                          ValueFormatDescriptorContainsSum);
  } else if (descriptor.IsStruct()) {
    return absl::c_any_of(descriptor.struct_elements(),
                          ValueFormatDescriptorContainsSum);
  }
  return false;
}
...
void DslxInterpreterEvents::AddTraceChannelMessage(
    ...
    ChannelDirection direction, const ValueFormatDescriptor& format_descriptor,
    bool redact_value) {
  std::string formatted_data;
  if (redact_value) {
    formatted_data = absl::StrCat(std::string(kChannelTraceIndentation, ' '),
                                  kOpaqueSemanticSumValue);
  } else {
    formatted_data = ToStringMaybeFormatted(value, format_descriptor,
...
/* static */ absl::StatusOr<InterpValue> BytecodeInterpreter::Interpret(
  ...
  if (options.trace_calls() && events.has_value()) {
    XLS_ASSIGN_OR_RETURN(
        std::vector<ValueFormatDescriptor> arg_descriptors,
...
absl::Status BytecodeInterpreter::Run(bool* progress_made) {
  ...
  while (!frames_.empty()) {
    ...
    if (options_.trace_calls() && events_.has_value() && source_fn != nullptr) {
      ...
      XLS_ASSIGN_OR_RETURN(
          ValueFormatDescriptor return_descriptor,
  ...
  switch (bytecode.op()) {
    ...
    case Bytecode::Op::kCreateSum: {
      ...
      XLS_ASSIGN_OR_RETURN(Phase1SumTypeEncoding::VariantInfo variant,
                           encoding.GetVariant(sum_data->variant_name()));
      XLS_ASSIGN_OR_RETURN(std::vector<InterpValue> payload_values,
                           PopArgsRightToLeft(variant.payload_size()));
      XLS_ASSIGN_OR_RETURN(
...
absl::Status BytecodeInterpreter::EvalAssertWellFormed(
  ...
  return ValidateWellFormedObserverValue(bytecode, *type, stack_.PeekOrDie(),
                                         "observer");
}
...
absl::Status BytecodeInterpreter::EvalCall(const Bytecode& bytecode) {
  ...
  if (user_fn_data.function->is_test_utility()) {
    ...
    // which is not a test.
    if (!is_init) {
      bool caller_is_test = IsInTestEntity(caller);
      if (!caller_is_test) {
  ...
  switch (item.kind()) {
    ...
    case Kind::kSum: {
      ...
      for (int64_t i = 0; i < sum_match.payload_items.size(); ++i) {
        XLS_ASSIGN_OR_RETURN(
            bool equal, MatchArmEqualsInterpValue(&frames_.back(),
    ...
    case Kind::kInvalidSum: {
      ...
      if (invalid_sum.raw_slot.has_value()) {
        frame->StoreSlot(*invalid_sum.raw_slot,
...
absl::Status BytecodeInterpreter::RunBuiltinFn(const Bytecode& bytecode,
  ...
  switch (builtin) {
    ...
    case Builtin::kFail: {
      ...
      XLS_ASSIGN_OR_RETURN(
          ValueFormatDescriptor value_format,
          MakeValueFormatDescriptor(*value_type, options_.format_preference()));
      std::string message = ValueFormatDescriptorContainsSum(value_format)
