...
BytecodeEmitter::EmitExpression(
  ...
  const Function* source_fn = GetContainingFunction(expr).value_or(nullptr);
...
absl::Status BytecodeEmitter::HandleBinop(const Binop* node) {
  ...
  switch (node->binop_kind()) {
    ...
    case BinopKind::kMod:
      ...
      break;
    case BinopKind::kEq:
      if (std::optional<Type*> lhs_type = type_info_->GetItem(node->lhs());
          lhs_type.has_value() && TypeContainsSum(**lhs_type)) {
        bytecode_.push_back(Bytecode(node->span(), Bytecode::Op::kEq,
                                     (*lhs_type)->CloneToUnique()));
      } else {
        bytecode_.push_back(Bytecode(node->span(), Bytecode::Op::kEq));
      }
      break;
...
absl::Status BytecodeEmitter::HandleBuiltinRecv(const Invocation* node) {
  ...
  // operand.
  XLS_ASSIGN_OR_RETURN(
      InterpValue default_value,
      CreateInternalPlaceholderValueFromType(channel_data.payload_type()));
...
absl::Status BytecodeEmitter::HandleSumConstructorInvocation(
  ...
  Add(Bytecode::MakeCreateSum(
      node->span(),
      Bytecode::SumConstructionData(sum_type.CloneToUnique(),
                                    std::string(constructor_ref->attr()))));
  return absl::OkStatus();
}
...
BytecodeEmitter::HandleSumVariantPayloadPattern(
    const SumVariantPayloadPattern* pattern, const SumType& sum_type) {
  const Phase1SumTypeEncoding encoding(sum_type);
  XLS_ASSIGN_OR_RETURN(Phase1SumTypeEncoding::VariantInfo variant,
                       encoding.GetVariant(pattern->constructor_ref()->attr()));

  std::vector<Bytecode::MatchArmItem> payload_items(
      variant.payload_size(), Bytecode::MatchArmItem::MakeWildcard());
  if (const auto* tuple = std::get_if<TuplePattern*>(&pattern->payload())) {
    ...
    XLS_RETURN_IF_ERROR(encoding.ForEachPayloadMember(
        variant,
        [&](int64_t active_index, const Type& slot_type) -> absl::Status {
          XLS_ASSIGN_OR_RETURN(
              payload_items[active_index],
              HandlePatternExpr(members.at(active_index),
    ...
    XLS_RETURN_IF_ERROR(encoding.ForEachPayloadMember(
        ...
        [&](int64_t active_index, const Type& slot_type) -> absl::Status {
          XLS_ASSIGN_OR_RETURN(
              payload_items[active_index],
              HandlePatternExpr(struct_payload_field_patterns.at(
    ...
    return absl::visit(
        Visitor{
            ...
            [&](WildcardPattern* n) -> absl::StatusOr<Bytecode::MatchArmItem> {
              if (n->kind() == AstNodeKind::kInvalidPattern) {
                ...
                auto* invalid_pattern = absl::down_cast<InvalidPattern*>(n);
                std::optional<Bytecode::SlotIndex> raw_slot = std::nullopt;
                if (invalid_pattern->raw_name_def() != nullptr) {
                  raw_slot = Bytecode::SlotIndex(next_slotno_++);
                  namedef_to_slot_[invalid_pattern->raw_name_def()] =
                      raw_slot->value();
                }
                return Bytecode::MatchArmItem::MakeInvalidSum(sum_type,
                                                              raw_slot);
              }
            ...
            [&](SumVariantPayloadPattern* constructor_pattern)
              ...
              XLS_RET_CHECK(sum_type != nullptr)
                  ...
                  << (type == nullptr ? "<null>" : type->ToString());
              return HandleSumVariantPayloadPattern(constructor_pattern,
                                                    *sum_type);
            },
            [&](RestOfTuple* n) -> absl::StatusOr<Bytecode::MatchArmItem> {
              return Bytecode::MatchArmItem::MakeRestOfTuple();
            },
            [&](StructPattern* n) -> absl::StatusOr<Bytecode::MatchArmItem> {
...
absl::Status BytecodeEmitter::HandleSumInstance(const SumInstance* node) {
  std::optional<const Type*> node_type = type_info_->GetItem(node);
  XLS_RET_CHECK(node_type.has_value());
  XLS_RET_CHECK((*node_type)->IsSum());
  ...
  Add(Bytecode::MakeCreateSum(
      node->span(), Bytecode::SumConstructionData(
                        sum_type.CloneToUnique(),
                        std::string(node->constructor_ref()->attr()))));
  return absl::OkStatus();
}
...
absl::Status BytecodeEmitter::HandleConditional(const Conditional* node) {
  // constexpr if selects a branch without producing additional bytecode,
  // by passing the visitor to the chosen branch.
  if (node->IsConst()) {
...
absl::Status BytecodeEmitter::HandleMatch(const Match* node) {
  ...
  if (TypeContainsSum(**type)) {
    Add(Bytecode::MakeAssertWellFormed(node->matched()->span(),
                                       (*type)->CloneToUnique()));
  }
