 ...
 private:
  ...
  absl::Status EmitLogic(absl::Span<Node* const> nodes,
    ...
    for (Node* node : nodes) {
      ...
      // Record residual entry at the start to avoid duplication.
      NodeResidualData* residual_meta = nullptr;
      if (block_residual_data_ != nullptr) {
        residual_meta = block_residual_data_->add_nodes();
        residual_meta->set_node_name(node->GetName());
        residual_meta->set_node_id(node->id());
        // Default to false; only set to true in the few inline cases.
        residual_meta->set_emitted_inline(false);
      }
      ...
      if (std::any_of(
        ...
        if (residual_meta != nullptr && !emit_as_assignment) {
          residual_meta->set_emitted_inline(true);
        }
        continue;
      }
