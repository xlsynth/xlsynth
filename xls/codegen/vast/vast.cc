...
static bool IsDigitsOrIdentifier(std::string_view s) {
  ...
  VerilogFunction::VerilogFunction(std::string_view name,
                                   DataType * result_type, VerilogFile * file,
                                   const SourceInfo& loc)
      : VastNode(file, loc),
        name_(name),
  ...
  std::string VerilogFunction::Emit(LineInfo * line_info) const {
    ...
    LineInfoIncrease(line_info, 1);
    LineInfoEnd(line_info, this);
    return absl::StrCat(absl::StrFormat("function automatic%s (%s);\n",
  ...
  absl::StatusOr<LogicRef*> Module::AddInput(
      std::string_view name, DataType * type, const SourceInfo& loc,
      DataKind data_kind) {
    XLS_RET_CHECK(data_kind == DataKind::kWire ||
                  data_kind == DataKind::kLogic);
    XLS_RETURN_IF_ERROR(NoteDefined(&defined_names_, name, "input"));
    return AddInputInternal(name, type, data_kind, loc);
  }

  absl::StatusOr<LogicRef*> Module::AddOutput(
      std::string_view name, DataType * type, const SourceInfo& loc,
      DataKind data_kind) {
    XLS_RET_CHECK(data_kind == DataKind::kWire ||
                  data_kind == DataKind::kLogic);
  ...
  absl::StatusOr<LogicRef*> Module::AddReg(
      std::string_view name, DataType * type, const SourceInfo& loc,
  ...
  absl::StatusOr<LogicRef*> Module::AddWire(
      std::string_view name, DataType * type, const SourceInfo& loc,
      ModuleSection* section) {
  ...
  absl::StatusOr<LogicRef*> Module::AddLogic(
      std::string_view name, DataType * type, const SourceInfo& loc,
      ModuleSection* section) {
  ...
  absl::StatusOr<LogicRef*> Module::AddWire(
      std::string_view name, DataType * type, Expression * init,
      const SourceInfo& loc, ModuleSection* section) {
    XLS_RETURN_IF_ERROR(NoteDefined(&defined_names_, name, "wire"));
    return AddWireInternal(name, type, init, loc, section);
  }

  LogicRef* Module::AddIntegerInternal(
      std::string_view name, const SourceInfo& loc, ModuleSection* section) {
    if (section == nullptr) {
      section = &top_;
    }
  ...
  absl::StatusOr<LogicRef*> Module::AddGenvar(
      std::string_view name, const SourceInfo& loc, ModuleSection* section) {
  ...
  ParameterRef* Module::AddParameterPort(
      std::string_view name, Expression * rhs, const SourceInfo& loc) {
    Parameter* param = file()->Make<Parameter>(loc, name, rhs);
  ...
  TypedefType* VerilogPackageSection::AddEnumTypedef(
      std::string_view name, Enum * enum_data_type, const SourceInfo& loc) {
  ...
  TypedefType* VerilogPackageSection::AddStructTypedef(
      std::string_view name, Struct * struct_data_type, const SourceInfo& loc) {
    Typedef* def = Add<Typedef>(
  ...
  TypedefType* VerilogPackageSection::AddStructTypedef(
      ...
      const SourceInfo& loc) {
    return AddStructTypedef(name, file()->Make<Struct>(loc, struct_members),
                            loc);
  }

  ParameterRef* VerilogPackageSection::AddParameter(
      std::string_view name, Expression * rhs, const SourceInfo& loc) {
    Parameter* param = file()->Make<Parameter>(loc, name, rhs);
  ...
  ArrayTypeBase::ArrayTypeBase(
      DataType * element_type, absl::Span<const int64_t> dims,
  ...
  PackedArrayType::PackedArrayType(
      Expression * width, absl::Span<Expression* const> packed_dims,
  ...
  PackedArrayType::PackedArrayType(
      int64_t width, absl::Span<const int64_t> packed_dims, bool is_signed,
      VerilogFile* file, const SourceInfo& loc)
      : ArrayTypeBase(file->Make<BitVectorType>(
                          loc, static_cast<int32_t>(width), is_signed),
                      packed_dims, /*dims_are_max=*/false, file, loc) {}

  PackedArrayType::PackedArrayType(
      DataType * element_type, absl::Span<const int64_t> packed_dims,
      bool dims_are_max, VerilogFile* file, const SourceInfo& loc)
  ...
  UnpackedArrayType::UnpackedArrayType(
      DataType * element_type, absl::Span<const int64_t> unpacked_dims,
      VerilogFile * file, const SourceInfo& loc)
  ...
  IntDef::IntDef(std::string_view name, VerilogFile * file,
                 const SourceInfo& loc)
  ...
  std::string EmitModuleMember(LineInfo* line_info,
  ...
  std::string Module::Emit(LineInfo * line_info) const {
    ...
    if (!parameter_ports_.empty()) {
      ...
      LineInfoIncrease(line_info, 1);
      absl::StrAppend(&result,
                      absl::StrJoin(parameter_ports_, ",\n  ",
                                    [=](std::string* out, Parameter* p) {
                                      absl::StrAppend(out, p->Emit(line_info));
                                    }));
      absl::StrAppend(&result, "\n)");
      if (!ports_.empty()) {
        absl::StrAppend(&result, " ");
  ...
  std::string Literal::Emit(LineInfo * line_info) const {
    ...
    std::string size_prefix = format_ == FormatPreference::kPlainHex
                                  ? ""
                                  : absl::StrCat(effective_bit_count_);
    const std::string raw_digits = BitsToRawDigits(
        bits_, FormatPreference::kHex,
  ...
  std::string Ternary::Emit(LineInfo * line_info) const {
    ...
    LineInfoStart(line_info, this);
    std::string result = absl::StrFormat(
        "%s ? %s : %s", maybe_paren_wrap(test_), maybe_paren_wrap(consequent_),
  ...
  std::string EnumMember::Emit(LineInfo * line_info) const {
    LineInfoStart(line_info, this);
    std::string result =
        absl::StrFormat("%s = %s", name_, rhs_->Emit(line_info));
    LineInfoEnd(line_info, this);
    return result;
  }
  ...
  std::string Case::Emit(LineInfo * line_info) const {
    ...
    for (auto& arm : arms_) {
      ...
      absl::StrAppend(
          &result, Indent(absl::StrFormat("%s: %s", arm_string, stmts_string)),
          "\n");
      LineInfoIncrease(line_info, 1);
    }
  ...
  MacroStatementBlock* StatementConditionalDirective::AddAlternate(
    ...
    alternates_.push_back({std::move(identifier),
                           file()->Make<MacroStatementBlock>(SourceInfo())});
    return alternates_.back().second;
  }
  ...
  std::string ModuleConditionalDirective::Emit(LineInfo * line_info) const {
    ...
    pieces.push_back(absl::StrCat(ConditionalDirectiveKindToString(kind_), " ",
                                  identifier_));
  ...
  RepeatStatement::RepeatStatement(Expression * repeat_count,
                                   VerilogFile * file, const SourceInfo& loc)
      : Statement(file, loc),
  ...
  std::string EmitSensitivityListElement(
      LineInfo* line_info, const SensitivityListElement& element) {
    return absl::visit(
  ...
  std::string AlwaysBase::Emit(LineInfo * line_info) const {
    ...
    LineInfoEnd(line_info, this);
    return absl::StrFormat("%s @ (%s) %s", name(), sensitivity_list,
                           statements);
  }
  ...
  std::string Instantiation::Emit(LineInfo * line_info) const {
    ...
    auto append_connection = [=](std::string* out,
                                 const Connection& parameter) {
