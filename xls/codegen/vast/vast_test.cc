...
TEST_P(VastTest, PackedUnionOverlaysEqualWidthMembers) {
  ...
  EXPECT_EQ(union_type->EmitWithIdentifier(nullptr, "payload"),
            R"(union packed {
  logic [15:0] bits;
  struct packed {
    logic [7:0] high;
...
TEST_P(VastTest, CommentAnnotations) {
  ...
  XLS_ASSERT_OK_AND_ASSIGN(
      ...
      m->AddReg("b", f.BitVectorType(8, SourceInfo()),
                SourceInfo(absl::MakeConstSpan(
                    {SourceLocation(Fileno(0), Lineno(3), Colno(1)),
                     SourceLocation(Fileno(1), Lineno(1), Colno(7))}))));
  XLS_ASSERT_OK_AND_ASSIGN(
      LogicRef * b_next,
...
TEST_P(VastTest, SrcAttributeAnnotations) {
  absl::flat_hash_map<Fileno, std::string> filemap = {
      {Fileno(0), std::string("test.x")},
      {Fileno(1), std::string("testlib.x")}};
  VerilogFile f(GetFileType(), AnnotationType::kSrcAttribute, filemap);
  ...
  (* src = "test.x:2.1-2.1" *)
  reg [7:0] a_next;
  (* src = "test.x:3.1-3.1|testlib.x:1.7-1.7" *)
  reg [7:0] b;
  (* src = "test.x:4.1-4.1" *)
  reg [7:0] b_next;
  (* src = "test.x:5.1-5.1" *)
