...
// We ported the test suite from the (now removed) proc inlining to check that
// block stitching supports the same features. Note that some changes needed to
// be made to allow blocks to codegen without cycles or deadlocks, and
// pipelining can change the order of some outputs (especially when using
...
TEST_F(BlockStitchingPassTest, SingleProc) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
  XLS_ASSERT_OK_AND_ASSIGN(
      Channel * ch_in,
...
TEST_F(BlockStitchingPassTest, NestedProcs) {
...
TEST_F(BlockStitchingPassTest, NestedProcsFifoDepth1) {
  // Nested procs where the inner proc does a trivial arithmetic operation.
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
  XLS_ASSERT_OK_AND_ASSIGN(
...
TEST_F(BlockStitchingPassTest, NestedProcsWithUnspecifiedFifoDepth) {
  // Nested procs where the inner proc does a trivial arithmetic operation.
...
TEST_F(BlockStitchingPassTest, NestedProcsWithNonzeroFifoDepth) {
  // Nested procs where the inner proc does a trivial arithmetic operation.
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
  XLS_ASSERT_OK_AND_ASSIGN(
...
TEST_F(BlockStitchingPassTest, NestedProcsWithSingleValue) {
  // Nested procs where the inner proc does a trivial arithmetic operation.
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
  XLS_ASSERT_OK_AND_ASSIGN(
...
TEST_F(BlockStitchingPassTest, NestedProcsWithConditionalSingleValueSend) {
...
TEST_F(BlockStitchingPassTest, NestedProcPassThrough) {
...
TEST_F(BlockStitchingPassTest, NestedProcDelayedPassThrough) {
...
TEST_F(BlockStitchingPassTest, InputPlusDelayedInput) {
...
TEST_F(BlockStitchingPassTest, NestedProcsTrivialInnerLoop) {
...
TEST_F(BlockStitchingPassTest, NestedProcsIota) {
...
TEST_F(BlockStitchingPassTest, NestedProcsOddIota) {
  // Output only nested procs where the inner proc just implements iota
...
TEST_F(BlockStitchingPassTest, SynchronizedNestedProcs) {
...
TEST_F(BlockStitchingPassTest, NestedProcsNontrivialInnerLoop) {
  // Nested procs where the inner proc loops more than once for each received
...
TEST_F(BlockStitchingPassTest, DoubleNestedProcsPassThrough) {
  // Nested procs where the inner proc passes through the value.
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
  XLS_ASSERT_OK_AND_ASSIGN(
...
TEST_F(BlockStitchingPassTest, SequentialNestedProcsPassThrough) {
...
TEST_F(BlockStitchingPassTest, SequentialNestedLoopingProcsWithState) {
...
TEST_F(BlockStitchingPassTest, SequentialNestedProcsWithLoops) {
...
TEST_F(BlockStitchingPassTest, DoubleNestedLoops) {
...
TEST_F(BlockStitchingPassTest, MultiIO) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
  XLS_ASSERT_OK_AND_ASSIGN(
      Channel * x_in,
...
TEST_F(BlockStitchingPassTest, NonTopProcsWithExternalStreamingIO) {
...
TEST_F(BlockStitchingPassTest, NonTopProcsWithExternalSingleValueIO) {
...
TEST_F(BlockStitchingPassTest, SingleValueAndStreamingChannels) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
  XLS_ASSERT_OK_AND_ASSIGN(
      Channel * x_in,
...
TEST_F(BlockStitchingPassTest, TriangleProcNetwork) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
  XLS_ASSERT_OK_AND_ASSIGN(
      Channel * ch_in,
...
TEST_F(BlockStitchingPassTest, DelayedReceiveWithDataLossFifoDepth0) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
  XLS_ASSERT_OK_AND_ASSIGN(
      Channel * ch_in,
...
TEST_F(BlockStitchingPassTest, DelayedReceiveWithNoDataLossFifoDepth1Variant0) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
  XLS_ASSERT_OK_AND_ASSIGN(
      Channel * ch_in,
...
TEST_F(BlockStitchingPassTest, DelayedReceiveWithNoDataLossFifoDepth1Variant1) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
  XLS_ASSERT_OK_AND_ASSIGN(
      Channel * ch_in,
...
TEST_F(BlockStitchingPassTest, DelayedReceiveWithDataLossFifoDepth1) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
  XLS_ASSERT_OK_AND_ASSIGN(
      Channel * ch_in,
...
TEST_F(BlockStitchingPassTest, DataLoss) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
  XLS_ASSERT_OK_AND_ASSIGN(
      Channel * ch_in,
...
TEST_F(BlockStitchingPassTest, BlockingReceiveBlocksSendsForDepth0Fifos) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
  XLS_ASSERT_OK_AND_ASSIGN(
      Channel * ch_in,
...
TEST_F(BlockStitchingPassTest, TwoSendsOneReceive) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
  XLS_ASSERT_OK_AND_ASSIGN(
      Channel * x_in,
...
TEST_F(BlockStitchingPassTest, TwoReceivesOneSend) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
  XLS_ASSERT_OK_AND_ASSIGN(
      Channel * x_in,
...
TEST_F(BlockStitchingPassTest, SingleValueChannelWithVariantElements1) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
...
TEST_F(BlockStitchingPassTest, SingleValueChannelWithVariantElements2) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
  Type* u64 = p->GetBitsType(64);
  Type* u32_u64 = p->GetTupleType({u32, u64});
...
TEST_F(BlockStitchingPassTest, SingleValueChannelWithVariantElements3) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
...
TEST_F(BlockStitchingPassTest, SingleValueChannelWithVariantElements4) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
  Type* u32_u32 = p->GetTupleType({u32, u32});
  XLS_ASSERT_OK_AND_ASSIGN(
...
TEST_F(BlockStitchingPassTest, TokenFanIn) {
...
TEST_F(BlockStitchingPassTest, TokenFanOut) {
...
TEST_F(BlockStitchingPassTest, RandomProcNetworks) {
...
TEST_F(BlockStitchingPassTest, DataDependencyWithoutTokenDependency) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
  XLS_ASSERT_OK_AND_ASSIGN(
      Channel * ch_in,
...
TEST_F(BlockStitchingPassTest, ReceivedValueSentAndNext) {
...
TEST_F(BlockStitchingPassTest, OffsetSendAndReceive) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
...
TEST_F(BlockStitchingPassTest, InliningProducesCycle) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
...
TEST_F(BlockStitchingPassTest, MultipleSends) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
...
TEST_F(BlockStitchingPassTest, MultipleSendsInDifferentOrder) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
...
TEST_F(BlockStitchingPassTest, MultipleReceivesFifoWithBypass) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
...
TEST_F(BlockStitchingPassTest, MultipleReceivesFifoDepth1) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
...
TEST_F(BlockStitchingPassTest, MultipleReceivesDoesNotFireEveryTick) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
...
TEST_F(BlockStitchingPassTest, MultipleReceivesDoesNotFireEveryTickFifoDepth0) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
...
TEST_F(BlockStitchingPassTest, MultipleSendsAndReceives) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
...
TEST_F(BlockStitchingPassTest, ReceiveIfsWithFalseCondition) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
...
TEST_F(BlockStitchingPassTest, ProcsWithDifferentII) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
  XLS_ASSERT_OK_AND_ASSIGN(
      Channel * ch_in,
...
TEST_F(BlockStitchingPassTest, ProcsWithNonblockingReceivesWithDroppingProc) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
...
TEST_F(BlockStitchingPassTest, ProcWithAssert) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
...
TEST_F(BlockStitchingPassTest, ProcWithGate) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
...
TEST_F(BlockStitchingPassTest, ProcWithTrace) {
  auto p = CreatePackage();
  Type* u32 = p->GetBitsType(32);
...
TEST_F(BlockStitchingPassTest, ProcWithConditionalNonblockingReceives) {
...
TEST_F(BlockStitchingPassTest, ProcWithExternalConditionalNonblockingReceives) {
  constexpr std::string_view ir_text = R"(package test
...
TEST_F(BlockStitchingPassTest, NestedProcsUsingEmptyAfterAll) {
