...
def _xls_pass_registry_impl(ctx):
    ...
    # macOS's libtool fails to create a static archive if the input object
    # file list is empty, even if we are just aggregating other static
    # libraries via `linking_contexts`. On Linux, `ar` silently creates an
    # empty archive, which is why this went unnoticed. To keep behaviour
    # identical across platforms, synthesise a one-line dummy translation
    # unit and compile it so that there is always at least one object to
    # archive if the compilation outputs are otherwise empty.
    if not comp_out.objects and not comp_out.pic_objects:
        dummy_src = ctx.actions.declare_file("%s_pass_registry_dummy.cc" % ctx.attr.name)
        ctx.actions.write(
            output = dummy_src,
            content = ("// Autogenerated stub to placate libtool on Darwin.\n" +
                       "int %s_pass_registry_dummy() { return 0; }\n") % ctx.attr.name,
        )
        (dummy_comp_ctx, dummy_comp_out) = cc_common.compile(
            name = ctx.label.name + "_dummy",
            actions = ctx.actions,
            feature_configuration = cc_features,
            cc_toolchain = cc_toolchain,
            srcs = [dummy_src],
        )
        comp_ctx = cc_common.merge_compilation_contexts(
            compilation_contexts = [comp_ctx, dummy_comp_ctx],
        )
        comp_out = cc_common.merge_compilation_outputs(
            compilation_outputs = [comp_out, dummy_comp_out],
        )
