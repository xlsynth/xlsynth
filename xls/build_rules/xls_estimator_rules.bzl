...
def _xls_default_estimator_models(ctx, reg, model_type):
    models = reg.models
    ...
    if not comp_out.objects and not comp_out.pic_objects:
        dummy_src = ctx.actions.declare_file("%s_%s_default_estimator_models_dummy.cc" % (ctx.attr.name, model_type))
        ctx.actions.write(
            output = dummy_src,
            content = ("// Autogenerated stub to placate libtool on Darwin.\n" +
                       "int %s_%s_default_estimator_models_dummy() { return 0; }\n") % (ctx.attr.name, model_type),
        )
...
def _xls_default_delay_models(ctx):
    ...
    return _xls_default_estimator_models(ctx, reg, "delay")
...
def _xls_default_area_models(ctx):
    ...
    return _xls_default_estimator_models(ctx, reg, "area")
